...
test('period tabs use one sliding selection indicator', () => {
  ...
  assert.match(html, /<nav class="tabs legacy-period-tabs"[\s\S]*?<span class="tab-indicator" aria-hidden="true"><\/span>[\s\S]*?data-period="today"[\s\S]*?data-period="month"[\s\S]*?data-period="allTime"/);
