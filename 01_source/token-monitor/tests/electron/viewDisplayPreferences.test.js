...
test('Expense is a reachable first-level view even while data collection is disabled', () => {
  const rendererSource = fs.readFileSync(path.join(__dirname, '../../src/electron/renderer/app.js'), 'utf8');
  const html = fs.readFileSync(path.join(__dirname, '../../src/electron/renderer/index.html'), 'utf8');
  const i18nSource = fs.readFileSync(path.join(__dirname, '../../src/electron/renderer/i18n.js'), 'utf8');
  const viewIds = extractViewIds(rendererSource, 'VIEW_DISPLAY_OPTIONS');

  assert.equal(viewIds.filter((id) => id === 'expense').length, 1);
  assert.match(rendererSource, /const order = \['home', baseBreakdownOrder\[0\], 'todo', 'expense'/);
  assert.doesNotMatch(rendererSource, /expenseEnabled !== true[^\n]+id !== 'expense'/);
  assert.match(rendererSource, /item\.dataset\.view = id/);
  assert.match(rendererSource, /else if \(state\.breakdown === 'expense'\)[\s\S]*?renderExpensePanel\(\)/);
  assert.match(html, /<section id="expensePanel" class="expense-panel hidden"><\/section>/);
  assert.equal((html.match(/id="expensePanel"/g) || []).length, 1);
  assert.match(i18nSource, /'views\.expense': '??'/);
});
