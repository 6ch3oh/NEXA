...
test('settingsForRenderer strips provider cookies before they reach the renderer', () => {
  ...
  assert.match(body, /credentialSettingsForRenderer\(settings, \{\s*omit: \['hubHostSecret', 'secret'\]\s*\}\)/);
  assert.match(body, /const rendererSettings = stripCredentialSettings\(settings \|\| \{\}\)/);
  assert.match(body, /hubCredentialConfigured: Boolean\(settings\?\.hubHostSecret\)/);
  assert.match(body, /clientCredentialConfigured: Boolean\(settings\?\.secret\)/);
