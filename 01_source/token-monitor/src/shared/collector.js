...
const { ensureTokscalePricingCacheFallback } = require('./tokscalePricingCacheFallback');
const {
  applyPeriodDelta,
  emptyPeriod,
...
async function spawnTokscaleJson(userArgs, commandTimeoutMs, deps = {}) {
  const preparePricingCache = deps.ensurePricingCacheFallback || ensureTokscalePricingCacheFallback;
  await preparePricingCache({ logger: (message) => console.log(message) });
  const command = deps.tokscaleCommand || tokscaleCommand;
  const spawnFn = deps.spawn || spawn;
  const { bin, prefixArgs, env } = command();
  return new Promise((resolve, reject) => {
    const child = spawnFn(bin, [...prefixArgs, ...userArgs], { env, windowsHide: true });
    let stdout = '';
    let stderr = '';
...
// does not (src/win/fs-event.c). An 8.3 short path such as <USER_PROFILE>\…
