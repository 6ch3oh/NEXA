...
const VIRTUAL_NAME_PATTERNS = [
  ...
  /vEthernet/i, /VMware/i, /VirtualBox/i, /Hyper-V/i, /WSL/i, /Radmin VPN/i,
  /\bVPN\b/i, /WireGuard/i, /Tailscale/i, /ZeroTier/i, /\bTAP\b/i, /\bTUN\b/i,
  /Loopback Pseudo/i
];
...
function lanIpv4Addresses() {
  return lanIpv4Interfaces().map(({ address, interface: interfaceName }) => ({
    address,
    interface: interfaceName
  }));
}

function lanIpv6Addresses() {
  const out = [];
  const seen = new Set();
  let nets;
  try { nets = os.networkInterfaces(); } catch (_) { return out; }
  for (const [name, addrs] of Object.entries(nets || {})) {
    ...
    for (const addr of addrs || []) {
      ...
      if (!addr || addr.family !== 'IPv6' || addr.internal || !address ||
        address === '::' || address === '::1' || address.startsWith('ff')) continue;
      if (isVirtualMac(addr.mac) || seen.has(address)) continue;
      seen.add(address);
      out.push({
        address,
        interface: name,
