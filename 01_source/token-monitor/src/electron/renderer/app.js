...
const expenseViewApi = window.TokenMonitorExpenseView;
...
const VIEW_DISPLAY_OPTIONS = [
  ...
  { id: 'todo', labelKey: 'views.todo' },
  { id: 'expense', labelKey: 'views.expense' }
];
const viewPeriodValues = new Set(['today', 'month', 'allTime']);
const viewBreakdownValues = new Set(['home', ...baseBreakdownOrder, 'status', 'limits', 'trends', 'todo', 'expense']);
...
const VIEW_ICON_CLASSES = {
  ...
  todo: 'view-icon-status',
  expense: 'view-icon-status'
};
...
const SETTINGS_SECTION_IDS = ['general', 'main', 'notion', 'expense', 'window', 'appearance', 'tools', 'limits', 'sync'];
const REFRESH_BUTTON_FEEDBACK_MS = 700;
// A manual (button) stats refresh must never spin forever: if the underlying
// getStats promise never settles, back out to an error state after this bound
// and free the button. Background/automatic refreshes are untouched.
const REFRESH_STATS_MAX_WAIT_MS = 45000;
// Expense reads can resolve in a few ms; keep the spinning cue visible just
// long enough to be read without dragging out any background expense work.
const REFRESH_EXPENSE_MIN_FEEDBACK_MS = 600;
...
Object.assign(els, {
  ...
  hubCredentialStatus: document.getElementById('hubCredentialStatus'),
  ...
  notionSettingsSummary: document.getElementById('notionSettingsSummary'),
  calendarLocalAiEnabledInput: document.getElementById('calendarLocalAiEnabledInput'),
  calendarLocalAiProviderIdInput: document.getElementById('calendarLocalAiProviderIdInput'),
  calendarLocalAiBaseUrlInput: document.getElementById('calendarLocalAiBaseUrlInput'),
  calendarLocalAiProtocolInput: document.getElementById('calendarLocalAiProtocolInput'),
  calendarLocalAiModelIdInput: document.getElementById('calendarLocalAiModelIdInput'),
  calendarLocalAiAuthorizationModeInput: document.getElementById('calendarLocalAiAuthorizationModeInput'),
  calendarLocalAiCredentialReferenceInput: document.getElementById('calendarLocalAiCredentialReferenceInput'),
  calendarLocalAiHealthEndpointInput: document.getElementById('calendarLocalAiHealthEndpointInput'),
  calendarLocalAiGenerationEndpointInput: document.getElementById('calendarLocalAiGenerationEndpointInput'),
  calendarLocalAiStreamingInput: document.getElementById('calendarLocalAiStreamingInput'),
  calendarLocalAiStructuredJsonInput: document.getElementById('calendarLocalAiStructuredJsonInput'),
  calendarLocalAiTimeoutInput: document.getElementById('calendarLocalAiTimeoutInput'),
  calendarLocalAiConcurrencyInput: document.getElementById('calendarLocalAiConcurrencyInput'),
  calendarLocalAiReadiness: document.getElementById('calendarLocalAiReadiness'),
  calendarLocalAiSaveButton: document.getElementById('calendarLocalAiSaveButton'),
  calendarLocalAiHealthButton: document.getElementById('calendarLocalAiHealthButton'),
  calendarLocalAiResetButton: document.getElementById('calendarLocalAiResetButton'),
  calendarLocalAiActionStatus: document.getElementById('calendarLocalAiActionStatus'),
...
function settingsSectionSummary(section) {
  ...
  if (section === 'notion') {
    if (!state.settings.notionTodoEnabled) return t('settings.summary.off');
    if (!state.settings.notionTokenConfigured || !state.settings.notionDataSourceId) return '未配置';
    return refreshIntervalLabel(state.settings.notionTodoRefreshMs);
  }
  if (section === 'window') {
  ...
  if (section === 'expense') {
    if (!state.settings.expenseEnabled) return t('settings.summary.off');
    return state.expense ? `${state.expense.totalCount ?? 0} ${t('settings.expense.rows')}` : '…';
  }
  return '';
}
...
function availableBreakdownIds() {
  const order = ['home', baseBreakdownOrder[0], 'todo', 'expense', 'status', 'trends', ...baseBreakdownOrder.slice(1)];
...
function openViewFromTray(viewId) {
  if (viewId?.source === 'mobile-global-command') {
    const routeId = typeof viewId.route_id === 'string' ? viewId.route_id.trim().toLowerCase() : '';
    const routeButton = [...document.querySelectorAll('[data-nexa-route]')]
      .find((button) => button.dataset.nexaRoute === routeId);
    if (routeButton) routeButton.click();
    const applied = [...document.querySelectorAll('[data-nexa-route]')]
      .some((button) => button.dataset.nexaRoute === routeId && button.getAttribute('aria-current') === 'page');
    window.tokenMonitor.setViewState?.({
...
function notionTodoMessage(snapshot) {
  if (!state.settings?.notionTodoEnabled || !state.settings?.notionTokenConfigured || !state.settings?.notionDataSourceId) return '尚未配置 Notion';
  if (state.notionTodoBusy) return '正在读取待办';
  return snapshot?.message || (snapshot?.items?.length ? '' : '暂无待办');
}

function formatNotionDue(due) {
  const start = due?.start || '';
  if (!start) return '无截止日期';
  if (/^\d{4}-\d{2}-\d{2}$/.test(start)) return start;
  const parsed = new Date(start);
  if (Number.isNaN(parsed.getTime())) return start;
  return parsed.toLocaleString([], due?.hasTime ? { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' } : { year: 'numeric', month: '2-digit', day: '2-digit' });
}

function notionTodoSectionTitle(key) {
  if (key === 'overdue') return '已逾期';
  if (key === 'today') return '今天';
  if (key === 'upcoming') return '未来 7 天';
  return '无日期';
}

function renderNotionTodoItem(todo) {
  const item = document.createElement('button');
  item.type = 'button';
  item.className = 'todo-item';
  item.dataset.url = todo.url || '';
  ...
  meta.textContent = [todo.priority || '无优先级', formatNotionDue(todo.due), todo.status || '无状态'].join(' · ');
  ...
  item.addEventListener('click', () => {
    if (todo.url) void window.tokenMonitor.notionTodo?.open?.(todo.url);
  });
  return item;
}
...
function renderTodoPanel() {
  ...
  const snapshot = state.notionTodo;
  const message = notionTodoMessage(snapshot);
  const header = document.createElement('div');
  header.className = 'todo-header';
  const stats = document.createElement('div');
  stats.className = 'todo-stats';
  const today = document.createElement('span');
  today.innerHTML = `<strong>${Number(snapshot?.todayCount || 0)}</strong><small>今日未完成</small>`;
  const overdue = document.createElement('span');
  overdue.innerHTML = `<strong>${Number(snapshot?.overdueCount || 0)}</strong><small>已逾期</small>`;
  stats.append(today, overdue);
  const actions = document.createElement('div');
  actions.className = 'todo-actions';
  const refreshed = document.createElement('span');
  refreshed.className = 'todo-refreshed';
  refreshed.textContent = snapshot?.updatedAt ? `最近刷新 ${formatTime(snapshot.updatedAt)}` : '尚未刷新';
  const refresh = document.createElement('button');
  refresh.type = 'button';
  refresh.className = 'todo-refresh-button';
  refresh.textContent = state.notionTodoBusy ? '正在读取待办' : '刷新';
  refresh.disabled = state.notionTodoBusy;
  refresh.addEventListener('click', () => void refreshNotionTodos({ force: true, feedback: true }));
  actions.append(refreshed, refresh);
  ...
  if (snapshot?.laterCount > 0) {
    ...
    later.className = 'todo-later-count';
    later.textContent = `更晚任务 ${snapshot.laterCount} 项`;
    body.append(later);
  }
  const sections = snapshot?.sections || {};
  for (const key of ['overdue', 'today', 'upcoming', 'nodate']) {
    const items = Array.isArray(sections[key]) ? sections[key] : [];
    if (items.length === 0) continue;
    const section = document.createElement('section');
    section.className = 'todo-section';
    const title = document.createElement('h2');
    title.textContent = notionTodoSectionTitle(key);
    section.append(title, ...items.map(renderNotionTodoItem));
    body.append(section);
  }
  els.todoPanel.replaceChildren(header, body);
}

function renderExpensePanel() {
  if (!els.expensePanel || !expenseViewApi) return;
  expenseViewApi.renderPanel(els.expensePanel, {
    snapshot: state.expense,
    locale: currentLocale()
  });
}

async function refreshExpense(snapshot) {
  if (!window.tokenMonitor.expense?.get) return;
  try {
    state.expense = snapshot || await window.tokenMonitor.expense.get();
  } catch (_) {
    state.expense = null;
  }
  if (state.breakdown === 'expense') renderExpensePanel();
  if (expenseViewApi && typeof expenseViewApi.syncSettings === 'function') {
    expenseViewApi.syncSettings({ ...(state.settings || {}), expense: state.expense, locale: currentLocale() });
  }
...
async function loadExpense() {
  if (!window.tokenMonitor.expense?.get) return;
  try {
    state.expense = await window.tokenMonitor.expense.get();
  } catch (_) {
    state.expense = null;
  }
}

async function refreshNotionTodos(options = {}) {
  if (state.notionTodoBusy) return state.notionTodo;
  if (!window.tokenMonitor.notionTodo?.refresh) return state.notionTodo;
  state.notionTodoBusy = true;
  if (options.feedback) setRefreshButtonState('refreshing');
  if (state.breakdown === 'todo') renderTodoPanel();
  try {
    state.notionTodo = await window.tokenMonitor.notionTodo.refresh();
    if (options.feedback) settleRefreshButtonState(state.notionTodo?.ok === false && !state.notionTodo?.items?.length ? 'error' : 'refreshed');
  } catch (_) {
    if (options.feedback) settleRefreshButtonState('error');
  } finally {
    state.notionTodoBusy = false;
    if (state.breakdown === 'todo') renderTodoPanel();
  }
  return state.notionTodo;
}

async function loadInitialNotionTodos() {
  try { state.notionTodo = await window.tokenMonitor.notionTodo?.get?.(); } catch (_) {}
  if (state.settings?.notionTodoEnabled && state.settings?.notionTokenConfigured && state.settings?.notionDataSourceId) {
    void refreshNotionTodos();
  }
}
function render() {
  ...
  renderTokenRate();
  if (!anyRefreshBusy() && !state.refreshFeedbackTimer) setRefreshButtonState('idle');
  ...
  els.todoPanel?.classList.toggle('hidden', state.breakdown !== 'todo');
  els.expensePanel?.classList.toggle('hidden', state.breakdown !== 'expense');
  if (state.breakdown === 'home') {
    ...
    els.expensePanel?.classList.add('hidden');
    els.homePanel.classList.remove('hidden');
    renderHome();
  } else if (state.breakdown === 'todo') {
    ...
    els.expensePanel?.classList.add('hidden');
    renderTodoPanel();
  } else if (state.breakdown === 'expense') {
    els.homePanel.classList.add('hidden');
    els.breakdown.classList.add('hidden');
    els.serviceStatusPanel?.classList.add('hidden');
    els.trendsPanel.classList.add('hidden');
...
//
// Each view (home/stats, expense, status) owns its own busy flag so a stuck
// collect in one view can never permanently freeze the button for the others.
// Every feedback refresh is stamped with a monotonic run id: whichever action
// acquires the button last (or times out) owns the feedback, and a stale task
// that completes late releases its own lock without overwriting the newer
// button state. The legacy state.refreshBusy is kept in sync as a combiner so
// existing render() logic and source guards stay meaningful.
// ---------------------------------------------------------------------------

// @refresh-controller-start
function setRefreshBusyFor(kind, value) {
  if (kind === 'stats') state.statsRefreshBusy = value;
  else if (kind === 'expense') state.expenseRefreshBusy = value;
  else if (kind === 'status') state.statusRefreshBusy = value;
  state.refreshBusy = state.statsRefreshBusy || state.expenseRefreshBusy || state.statusRefreshBusy;
}

function refreshBusyFor(kind) {
  if (kind === 'stats') return state.statsRefreshBusy;
  if (kind === 'expense') return state.expenseRefreshBusy;
  if (kind === 'status') return state.statusRefreshBusy;
  return false;
}

function anyRefreshBusy() {
  return state.statsRefreshBusy || state.expenseRefreshBusy || state.statusRefreshBusy;
}

// Reserves the view's lock and hands back its run id, or null when that view is
// already mid-refresh (duplicate clicks on the same view are dropped). Run ids
// are per-view, so activity in one view never invalidates another's. The caller
// then drives the button with its own setRefreshButtonState('refreshing').
function acquireRefresh(kind) {
  if (refreshBusyFor(kind)) return null;
  state[`${kind}RefreshRunId`] += 1;
  setRefreshBusyFor(kind, true);
  return state[`${kind}RefreshRunId`];
}

// True while the given run is still the active run of ITS OWN view: the view
// is busy and no timeout/newer run of that view has taken over. Guards both the
// lock release and the settle, so a stale task (timed out or superseded) can
// never clear a newer run's lock or overwrite its button state.
function isCurrentRefresh(kind, runId) {
  return refreshBusyFor(kind) && runId === state[`${kind}RefreshRunId`];
}

// Bounds a hung refresh: invalidates the in-flight run so a late completion is
// ignored, frees the button, and surfaces an error reading.
function expireRefresh(kind, runId) {
  if (!isCurrentRefresh(kind, runId)) return;
  state[`${kind}RefreshRunId`] += 1;
  setRefreshBusyFor(kind, false);
  settleRefreshButtonState('error');
}

// Keeps the spinning cue visible for at least minMs without delaying the data
// work itself — the extra wait only extends the *visible feedback*.
function waitForMinRefreshFeedback(startedAt, minMs) {
...
async function refreshStats(options = {}) {
  ...
  const runId = feedback ? acquireRefresh('stats') : null;
  if (feedback && runId === null) return;
  if (feedback) {
    clearRefreshButtonFeedbackTimer();
    setRefreshButtonState('refreshing');
  }
  // Only a manual (button) refresh is bounded: a hung getStats must never leave
  // the button spinning forever. Background/interval refreshes (feedback ===
  // false) keep the old unbounded behaviour.
  const timeout = feedback ? setTimeout(() => {
    expireRefresh('stats', runId);
  }, REFRESH_STATS_MAX_WAIT_MS) : null;
  try {
    ...
    maybeUpdateBarsIcon();
    if (feedback) {
      // Only the still-current run may release its own lock or settle: a stale
      // run (already expired or superseded) must leave the newer run untouched.
      if (isCurrentRefresh('stats', runId)) {
        setRefreshBusyFor('stats', false);
...
async function refreshExpenseManually() {
  if (!window.tokenMonitor.expense?.get) return;
  const runId = acquireRefresh('expense');
  ...
  try {
    ...
    // Local expense reads can complete in a few ms; keep the spinning cue
    // visible for a readable minimum without slowing down the read itself.
    await waitForMinRefreshFeedback(startedAt, REFRESH_EXPENSE_MIN_FEEDBACK_MS);
    if (isCurrentRefresh('expense', runId)) {
      setRefreshBusyFor('expense', false);
      settleRefreshButtonState(state.expense ? 'refreshed' : 'error');
    }
  } catch (_) {
    await waitForMinRefreshFeedback(startedAt, REFRESH_EXPENSE_MIN_FEEDBACK_MS);
    if (isCurrentRefresh('expense', runId)) {
...
async function refreshStatusViewManually() {
  if (state.serviceStatusBusy) return;
  const runId = acquireRefresh('status');
  if (runId === null) return;
  clearRefreshButtonFeedbackTimer();
  setRefreshButtonState('refreshing');
  try {
    await refreshServiceStatus({ force: true });
    if (isCurrentRefresh('status', runId)) {
      setRefreshBusyFor('status', false);
      settleRefreshButtonState('refreshed');
    }
  } catch (error) {
...
function syncHubModeUi() {
  ...
  if (mode === 'host') {
    ...
    renderHubCredentialStatus();
    renderHubStatus();
  }
  renderSyncClientStatus();
}

function renderHubCredentialStatus() {
  if (!els.hubCredentialStatus) return;
  const configured = state.hubInfo?.credentialConfigured === true
    || state.settings?.hubCredentialConfigured === true;
  els.hubCredentialStatus.textContent = configured
    ? t('settings.sync.credentialConfigured')
    : t('settings.sync.credentialNotConfigured');
  els.hubCredentialStatus.classList.toggle('configured', configured);
}
...
let calendarLocalAiHealthBusy = false;

function calendarLocalAiConfigApi() {
  return window.CalendarLocalAiConfig;
}

function calendarLocalAiConfigurationFromForm() {
  const authMode = els.calendarLocalAiAuthorizationModeInput?.value || 'none';
  return calendarLocalAiConfigApi().normalizeCalendarLocalAiConfiguration({
    version: 1,
    enabled: els.calendarLocalAiEnabledInput?.checked !== false,
    providerId: els.calendarLocalAiProviderIdInput?.value,
    baseUrl: els.calendarLocalAiBaseUrlInput?.value,
    protocol: els.calendarLocalAiProtocolInput?.value,
    modelId: els.calendarLocalAiModelIdInput?.value,
    authorizationMode: authMode,
    credentialReference: authMode === 'bearer-env' ? els.calendarLocalAiCredentialReferenceInput?.value : '',
    healthCheckEndpoint: els.calendarLocalAiHealthEndpointInput?.value,
    generationEndpoint: els.calendarLocalAiGenerationEndpointInput?.value,
    streamingCapability: els.calendarLocalAiStreamingInput?.value,
    structuredJsonCapability: els.calendarLocalAiStructuredJsonInput?.value,
    timeoutMs: els.calendarLocalAiTimeoutInput?.value,
    maxConcurrency: els.calendarLocalAiConcurrencyInput?.value
  });
}

function renderCalendarLocalAiConfiguration() {
  if (!els.calendarLocalAiEnabledInput || !calendarLocalAiConfigApi()) return;
  const api = calendarLocalAiConfigApi();
  const configuration = api.normalizeCalendarLocalAiConfiguration(
    state.settings?.calendarLocalAiConfiguration || api.defaultCalendarLocalAiConfiguration()
  );
  els.calendarLocalAiEnabledInput.checked = configuration.enabled;
  els.calendarLocalAiProviderIdInput.value = configuration.providerId;
  els.calendarLocalAiBaseUrlInput.value = configuration.baseUrl;
  els.calendarLocalAiProtocolInput.value = configuration.protocol;
  els.calendarLocalAiModelIdInput.value = configuration.modelId;
  els.calendarLocalAiAuthorizationModeInput.value = configuration.authorizationMode;
  els.calendarLocalAiCredentialReferenceInput.value = configuration.credentialReference;
  els.calendarLocalAiCredentialReferenceInput.disabled = configuration.authorizationMode !== 'bearer-env';
  els.calendarLocalAiHealthEndpointInput.value = configuration.healthCheckEndpoint;
  els.calendarLocalAiGenerationEndpointInput.value = configuration.generationEndpoint;
  els.calendarLocalAiStreamingInput.value = configuration.streamingCapability;
  els.calendarLocalAiStructuredJsonInput.value = configuration.structuredJsonCapability;
  els.calendarLocalAiTimeoutInput.value = String(configuration.timeoutMs);
  els.calendarLocalAiConcurrencyInput.value = String(configuration.maxConcurrency);
  const readiness = api.calendarLocalAiReadiness(configuration);
  const labels = {
    unconfigured: '共享本地 AI 尚未配置',
    disabled: '共享本地 AI 已停用',
    configuration_incomplete: `配置不完整 · 还需确认 ${readiness.issues.length} 项`,
    ready_for_health_check: '配置完整 · 等待健康检查'
  };
  els.calendarLocalAiReadiness.textContent = labels[readiness.status] || '配置状态未知';
  els.calendarLocalAiHealthButton.disabled = calendarLocalAiHealthBusy || !readiness.canHealthCheck;
  els.calendarLocalAiHealthButton.textContent = calendarLocalAiHealthBusy ? '正在检查…' : '检查本地模型';
}

const SHARED_LOCAL_AI_FAILURE_LABELS = Object.freeze({
  SERVER_NOT_RUNNING: '本地服务未运行',
  MODEL_NOT_LOADED: '本地服务未加载模型',
  MODEL_ID_MISMATCH: '已加载模型与配置的 Model ID 不一致',
  STRUCTURED_OUTPUT_UNAVAILABLE: '严格结构化输出不可用',
  TIMEOUT: '本地模型请求超时',
  INVALID_RESPONSE: '本地服务返回无效响应',
  AI_CONFIGURATION_INCOMPLETE: '配置尚不完整，未发起网络请求',
});

function sharedLocalAiHealthSummary(result) {
  const diagnostics = result?.diagnostics || {};
  return [
    `Runtime ${diagnostics.runtime || '未知'}`,
    `服务 ${diagnostics.service_status || '未知'}`,
    `Model ${diagnostics.model_id || '未知'}`,
    `localhost ${diagnostics.localhost_only === true ? 'YES' : 'NO'}`,
    `Structured ${diagnostics.structured_output || '未知'} / schema ${diagnostics.schema_validated === true ? 'PASS' : 'FAIL'}`,
    `Calendar ${diagnostics.calendar_readiness || '未知'}`,
    `Expense ${diagnostics.expense_readiness || '未知'}`,
    `Global Command ${diagnostics.global_command_readiness || '未知'}`,
    `延迟 ${Number(diagnostics.response_latency_ms ?? result?.latencyMs) || 0} ms`,
  ].join(' · ');
}

async function saveCalendarLocalAiConfiguration() {
  const configuration = calendarLocalAiConfigurationFromForm();
  if (els.calendarLocalAiActionStatus) els.calendarLocalAiActionStatus.textContent = '正在保存连接元数据…';
  try {
    await saveSettings({ calendarLocalAiConfiguration: configuration });
    const readiness = calendarLocalAiConfigApi().calendarLocalAiReadiness(state.settings.calendarLocalAiConfiguration);
    if (els.calendarLocalAiActionStatus) {
      els.calendarLocalAiActionStatus.textContent = readiness.canHealthCheck
        ? '配置已保存。尚未调用模型；可执行本地健康检查。'
        : '配置已保存，但信息尚不完整；当前模型调用为 0。';
    }
  } catch (_) {
    if (els.calendarLocalAiActionStatus) els.calendarLocalAiActionStatus.textContent = '配置保存失败，旧配置保持不变。';
  }
}

async function checkCalendarLocalAiHealth() {
  const checkHealth = window.tokenMonitor.nexa?.['today-tomorrow']?.checkLocalAiHealth;
  if (calendarLocalAiHealthBusy || typeof checkHealth !== 'function') return;
  calendarLocalAiHealthBusy = true;
  if (els.calendarLocalAiActionStatus) els.calendarLocalAiActionStatus.textContent = '正在用合成 fixture 检查共享本地模型，不会读取或写入日历、消费内容…';
  renderCalendarLocalAiConfiguration();
  try {
    const envelope = await checkHealth();
    const result = envelope?.ok === true && Object.hasOwn(envelope, 'value') ? envelope.value : envelope;
    if (els.calendarLocalAiActionStatus) {
      els.calendarLocalAiActionStatus.textContent = result?.ok
        ? sharedLocalAiHealthSummary(result)
        : `${result?.code || 'INVALID_RESPONSE'} · ${SHARED_LOCAL_AI_FAILURE_LABELS[result?.code] || '共享本地模型暂不可用'}；未发生日历或消费写入。`;
    }
  } catch (_) {
    if (els.calendarLocalAiActionStatus) els.calendarLocalAiActionStatus.textContent = 'INVALID_RESPONSE · 健康检查未完成；未发生日历或消费写入。';
  } finally {
    calendarLocalAiHealthBusy = false;
    renderCalendarLocalAiConfiguration();
  }
}

async function resetCalendarLocalAiConfiguration() {
  if (els.calendarLocalAiActionStatus) els.calendarLocalAiActionStatus.textContent = '正在清空本地模型连接信息…';
  try {
    await saveSettings({ calendarLocalAiConfiguration: calendarLocalAiConfigApi().defaultCalendarLocalAiConfiguration() });
    if (els.calendarLocalAiActionStatus) els.calendarLocalAiActionStatus.textContent = '连接信息已清空；当前模型调用为 0。';
  } catch (_) {
    if (els.calendarLocalAiActionStatus) els.calendarLocalAiActionStatus.textContent = '清空失败，旧配置保持不变。';
  }
}
...
function syncSettingsForm() {
  ...
  els.secretInput.placeholder = state.settings.clientCredentialConfigured
    ? t('settings.sync.credentialConfigured')
    : t('settings.sync.secretPlaceholder');
  ...
  if (els.windowSizeModeInput) {
    els.windowSizeModeInput.value = ['compact', 'standard', 'expanded', 'manual'].includes(state.settings.windowSizeMode)
      ? state.settings.windowSizeMode
      : 'standard';
  }
  ...
  if (els.notionTodoEnabledInput) els.notionTodoEnabledInput.checked = Boolean(state.settings.notionTodoEnabled);
  if (els.notionDataSourceIdInput) els.notionDataSourceIdInput.value = state.settings.notionDataSourceId || '6c2b41b5-a595-4b61-817f-a9019550392b';
  if (els.notionTodoRefreshInput) els.notionTodoRefreshInput.value = String(state.settings.notionTodoRefreshMs || 300000);
  if (els.notionTokenInput) els.notionTokenInput.value = '';
  if (els.notionTokenStatus) els.notionTokenStatus.textContent = state.settings.notionTokenConfigured ? '已保存' : '未保存';
  renderCalendarLocalAiConfiguration();
...
async function init() {
  ...
  window.tokenMonitor.notionTodo?.onPush?.((payload) => {
    state.notionTodo = payload;
    if (state.breakdown === 'todo') renderTodoPanel();
  });
  window.tokenMonitor.expense?.onPush?.((payload) => {
    state.expense = payload;
    if (state.breakdown === 'expense') renderExpensePanel();
    if (expenseViewApi && typeof expenseViewApi.syncSettings === 'function') {
      expenseViewApi.syncSettings({ ...(state.settings || {}), expense: payload, locale: currentLocale() });
    }
  });
  expenseViewApi?.setLocale?.(() => currentLocale());
  expenseViewApi?.bindSettings?.({
    onRefresh: () => { void refreshExpense(); },
    locale: currentLocale()
  });
...
els.saveSettingsButton.addEventListener('click', async () => {
  ...
  if (els.secretInput.value) patch.secret = els.secretInput.value;
...
els.hubSecretRegenButton?.addEventListener('click', async () => {
  ...
  state.hubInfo = info;
  state.settings = { ...state.settings, hubCredentialConfigured: info.credentialConfigured === true };
  renderHubCredentialStatus();
  renderHubStatus();
});
...
els.notionTodoEnabledInput?.addEventListener('change', async () => {
  await saveSettings({ notionTodoEnabled: els.notionTodoEnabledInput.checked });
  await refreshNotionTodos({ force: true });
});
els.notionDataSourceIdInput?.addEventListener('change', async () => {
  await saveSettings({ notionDataSourceId: els.notionDataSourceIdInput.value.trim() });
  await refreshNotionTodos({ force: true });
});
els.notionTodoRefreshInput?.addEventListener('change', async () => {
  await saveSettings({ notionTodoRefreshMs: Number(els.notionTodoRefreshInput.value) });
});
els.notionTokenSaveButton?.addEventListener('click', async () => {
  const token = els.notionTokenInput?.value || '';
  if (!token.trim()) {
    if (els.notionTestStatus) els.notionTestStatus.textContent = 'Token 为空';
    return;
  }
  await saveSettings({ notionToken: token });
  if (els.notionTokenInput) els.notionTokenInput.value = '';
  await refreshNotionTodos({ force: true });
});
els.notionTokenClearButton?.addEventListener('click', async () => {
  await saveSettings({ notionToken: '' });
  if (els.notionTokenInput) els.notionTokenInput.value = '';
  await refreshNotionTodos({ force: true });
});
els.notionTestButton?.addEventListener('click', async () => {
  if (els.notionTestStatus) els.notionTestStatus.textContent = '正在读取待办';
  const token = els.notionTokenInput?.value || '';
  const patch = { notionDataSourceId: els.notionDataSourceIdInput?.value?.trim?.() || '' };
  if (token.trim()) patch.notionToken = token;
  await saveSettings(patch);
  const result = await window.tokenMonitor.notionTodo?.test?.();
  if (els.notionTokenInput) els.notionTokenInput.value = '';
  if (els.notionTestStatus) els.notionTestStatus.textContent = result?.ok ? '连接成功' : (result?.message || '网络连接失败');
});
els.calendarLocalAiAuthorizationModeInput?.addEventListener('change', () => {
  els.calendarLocalAiCredentialReferenceInput.disabled = els.calendarLocalAiAuthorizationModeInput.value !== 'bearer-env';
});
els.calendarLocalAiSaveButton?.addEventListener('click', saveCalendarLocalAiConfiguration);
els.calendarLocalAiHealthButton?.addEventListener('click', checkCalendarLocalAiHealth);
els.calendarLocalAiResetButton?.addEventListener('click', resetCalendarLocalAiConfiguration);
els.windowBehaviorInput.addEventListener('change', () => saveSettings({ windowBehavior: els.windowBehaviorInput.value }));
els.windowSizeModeInput?.addEventListener('change', () => saveSettings({ windowSizeMode: els.windowSizeModeInput.value }));
...
els.refreshButton.addEventListener('click', () => {
  if (state.breakdown === 'todo') refreshNotionTodos({ force: true, feedback: true }).catch(() => {});
  else if (state.breakdown === 'status') refreshStatusViewManually().catch(() => {});
  else if (state.breakdown === 'expense') refreshExpenseManually().catch(() => {});
...
els.maxButton.addEventListener('click', () => window.tokenMonitor.nexa.control.toggleMaximize());
...
window.tokenMonitor.onSettingsPush?.((next) => {
  ...
  const prevNotionEnabled = state.settings?.notionTodoEnabled;
  const prevNotionConfigured = state.settings?.notionTokenConfigured && state.settings?.notionDataSourceId;
  state.settings = next;
  applyEffectiveCurrencyRates();
  syncSettingsForm();
  maybeUpdateBarsIcon();
  if (prevNotionEnabled !== next.notionTodoEnabled || Boolean(prevNotionConfigured) !== Boolean(next.notionTokenConfigured && next.notionDataSourceId)) {
    void refreshNotionTodos({ force: true });
  }
...
window.tokenMonitor.onHubPush?.((payload) => {
  ...
  state.settings = {
    ...state.settings,
    hubCredentialConfigured: payload.info.credentialConfigured === true
  };
  renderHubCredentialStatus();
  renderHubStatus();
});
