...
const { defaultDeviceId, generateHubSecret, lanIpv4Addresses, lanIpv4Interfaces, lanIpv6Addresses, loadDotEnv, pidFilePath, sharedDataDir } = require('../shared/config');
const {
  CredentialStore,
...
const { createMobileSyncStore } = require('../shared/mobileSyncStore');
const {
...
const { createCalendarLocalAiProviderRuntime } = require('./calendarLocalAiProvider');
const { createSharedLocalAiRuntimeCoordinator } = require('./sharedLocalAiRuntimeCoordinator');
const { createNexaCommandAuditLog } = require('./nexaCommandAuditLog');
const { projectMobileEventToExpenseDraft } = require('./mobileExpenseDraftBridge');
const { createMobileDeviceCredentialAuthority } = require('../shared/mobileDeviceCredentialAuthority');
const { createMobilePairingRuntime, rotateMobileTlsIdentity } = require('./mobilePairingRuntime');
const { createMobileReverseLanTransport, REVERSE_LAN_PORT, TRANSPORT_MODE } = require('./mobileReverseLanTransport');
const { createMobileRelayTransport, RELAY_TRANSPORT_STATES } = require('./mobileRelayTransport');
const { createMobileDeviceControlPlane } = require('./mobileDeviceControlPlane');
const { createDeviceControlClient } = require('./deviceControlClient');
const { createDesktopDdnsRuntime } = require('./desktopDdnsRuntime');
const { createMobileAwarenessReadModel } = require('./mobileAwarenessReadModel');
const { createMobileRuntimeBundleApplication } = require('./mobileRuntimeBundle');
const { createMobileProductGateway } = require('./mobileProductGateway');
const { createMobileNotificationLedger, temporalProjection } = require('./mobileNotificationLedger');
const { createNexaTemporalIndex } = require('./nexaTemporalIndex');
const { createLocalSpeechToTextProvider } = require('./localSpeechToTextProvider');
const {
  createDesktopSelfStatus,
  desktopIdentity
} = require('../shared/deviceAwarenessProtocol');
const { windowsArpPeerCandidates } = require('./mobileLanPeerCandidates');
const { discoverReverseLanPeers } = require('./mobileReverseLanDiscovery');
const { installSafeStdout } = require('../shared/safeStdio');
const { appVersion } = require('../shared/appVersion');
const { stripHubCredentialFields } = require('./hubRendererSecurity');
...
} = require('../shared/notionTodo');
const {
  DEFAULT_CATEGORY_RULES
} = require('../shared/expense');
const { createExpenseRuntime, DEFAULT_INBOX_NAME } = require('./expenseRuntime');
...
const { createEmptyNexaElectronShellBridge } = require('./nexaShellBridge');
const { createNexaAppComposition } = require('./nexaAppComposition');
const { createNexaDeviceCenterActivityController } = require('./nexaDeviceCenterActivity');
const { applyNexaIpcRegistrationPlan } = require('./nexaIpcRegistration');
...
const DEFAULT_WINDOW = { ...WINDOW_SIZE_PRESETS[WINDOW_SIZE_MODES.STANDARD] };
const WINDOW_LIMITS = { minWidth: 240, minHeight: 140, maxWidth: 1600, maxHeight: 1400 };
...
const CSP_HEADER = [
  ...
  "font-src 'self'",
  "connect-src 'self'",
  "frame-src http://127.0.0.1:4316 http://127.0.0.1:4416 http://127.0.0.1:4516 http://127.0.0.1:4616 http://127.0.0.1:4816",
  "object-src 'none'",
  "base-uri 'none'",
  "form-action 'none'",
...
const DEFAULT_VIEW_LIST = ['home', 'tool', 'status', 'device', 'model', 'project', 'session', 'limits', 'trends', 'todo', 'expense'].map((id) => ({ id }));
...
const NEXA_MOBILE_COMMAND_ROUTE_IDS = new Set([
...
let windowSizeMigrationPending = false;
let credentialStore = null;
let credentialStorageErrorShown = false;
let notionTodoRuntime = null;
let expenseRuntime = null;
let sessionUsageArchive = null;
let calendarLocalAiProviderRuntime = null;
let localAiRuntimeCoordinator = null;
let nexaCommandAuditLog = null;
let rendererViewState = normalizeInitialRendererViewState();
const serviceStatusClient = createServiceStatusClient();
let nexaAppComposition = null;
let nexaShellBridge = null;
let nexaIpcRegistrationHandle = null;
let nexaShellStartPromise = null;
const nexaDeviceCenterActivityController = createNexaDeviceCenterActivityController({
  getComposition: () => nexaAppComposition,
  getMainWindow: () => mainWindow,
  onError: (code) => console.log(`[device-center] activity cadence sync failed: ${code}`)
});
...
function defaultSettings() {
  ...
  return {
    ...
    notionToken: '',
    notionDataSourceId: DEFAULT_NOTION_TODO_DATA_SOURCE_ID,
    notionTodoRefreshMs: DEFAULT_NOTION_TODO_REFRESH_MS,
    expenseEnabled: parseBoolean(process.env.TOKEN_MONITOR_EXPENSE_ENABLED, false),
    expenseAutoCategorize: parseBoolean(process.env.TOKEN_MONITOR_EXPENSE_AUTO_CATEGORIZE, true),
    expenseDefaultCurrency: normalizeCurrency(process.env.TOKEN_MONITOR_EXPENSE_CURRENCY || 'CNY'),
    expenseInboxRoot: process.env.TOKEN_MONITOR_EXPENSE_INBOX_ROOT || '',
    ...
    language: 'auto',
    nexaModuleControl: { version: 1, modules: {} },
    calendarLocalAiConfiguration: defaultCalendarLocalAiConfiguration(),
    claudeWebCookie: '',
    opencodeCookie: '',
    opencodeProfiles: {},
    openrouterProfiles: {},
...
function restoredBounds() {
  ...
  if (!validWindowBounds(saved)) return null;
  return boundsForWindowSizeMode(
    settings?.windowSizeMode,
    saved,
    settings?.manualWindowBounds,
    workAreaForBounds(saved),
    WINDOW_LIMITS
  );
}

let persistBoundsTimer = null;
let persistBoundsResizeObserved = false;
let applyingWindowSizeMode = false;
let expectedProgrammaticWindowBounds = null;
...
function stopPersistBoundsTimer() {
  ...
  persistBoundsResizeObserved = false;
}

function workAreaForBounds(bounds) {
  try {
    const valid = validWindowBounds(bounds);
    const display = valid && typeof valid.x === 'number'
      ? screen.getDisplayMatching(valid)
      : screen.getPrimaryDisplay();
    if (display?.workArea) return display.workArea;
  } catch (_) {}
  return { x: 0, y: 0, width: WINDOW_LIMITS.maxWidth, height: WINDOW_LIMITS.maxHeight };
}

function preparedWindowBounds(mode) {
  const normalizedMode = normalizeWindowSizeMode(mode);
  const current = mainWindow && !mainWindow.isDestroyed()
    ? validWindowBounds(normalWindowBounds(mainWindow)) || validWindowBounds(mainWindow.getBounds())
    : validWindowBounds(settings?.windowBounds) || DEFAULT_WINDOW;
  return boundsForWindowSizeMode(
    normalizedMode,
    current,
    settings?.manualWindowBounds,
    workAreaForBounds(current),
    WINDOW_LIMITS
  );
}

function prepareWindowSizeMode(mode) {
  const normalizedMode = normalizeWindowSizeMode(mode);
  const bounds = preparedWindowBounds(normalizedMode);
  settings.windowSizeMode = normalizedMode;
  settings.windowMigrationDone = true;
  settings.windowBounds = bounds;
  if (normalizedMode === WINDOW_SIZE_MODES.MANUAL) settings.manualWindowBounds = bounds;
  return bounds;
}

function setMainWindowBoundsPreservingMaximize(bounds) {
  if (!mainWindow || mainWindow.isDestroyed() || settings?.trayMode || floatingBubbleState.collapsed) return;
  const wasMaximized = mainWindow.isMaximized();
  applyingWindowSizeMode = true;
  expectedProgrammaticWindowBounds = { ...bounds };
  const expected = expectedProgrammaticWindowBounds;
  ...
  try {
    ...
    setTimeout(() => { applyingWindowSizeMode = false; }, 1200);
    setTimeout(() => {
      if (expectedProgrammaticWindowBounds === expected) expectedProgrammaticWindowBounds = null;
    }, 2500);
  }
}
...
function clampMainWindowToVisibleWorkArea() {
  if (!mainWindow || mainWindow.isDestroyed() || mainWindow.isMinimized() || settings?.trayMode || floatingBubbleState.collapsed) return;
  const current = validWindowBounds(normalWindowBounds(mainWindow)) || validWindowBounds(mainWindow.getBounds());
  if (!current) return;
  const next = settings.windowSizeMode === WINDOW_SIZE_MODES.MANUAL
    ? clampWindowBounds(current, workAreaForBounds(current), WINDOW_LIMITS)
    : boundsForWindowSizeMode(settings.windowSizeMode, current, null, workAreaForBounds(current), WINDOW_LIMITS);
  ...
  if (settings.windowSizeMode === WINDOW_SIZE_MODES.MANUAL) settings.manualWindowBounds = next;
  saveSettings();
  setMainWindowBoundsPreservingMaximize(next);
}
...
function ensureSettingsLoaded() {
  ...
  if (windowSizeMigrationPending && saveSettings()) windowSizeMigrationPending = false;
...
function persistBoundsSoon(options = {}) {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  if (!shouldPersistWindowBounds(mainWindow)) {
    stopPersistBoundsTimer();
    return;
  }
  const resizeObserved = persistBoundsResizeObserved || options.resized === true;
  stopPersistBoundsTimer();
  persistBoundsResizeObserved = resizeObserved;
  persistBoundsTimer = setTimeout(() => {
    persistBoundsTimer = null;
    if (!mainWindow || mainWindow.isDestroyed()) return;
    if (!shouldPersistWindowBounds(mainWindow)) return;
    const next = mainWindow.getBounds();
    const resized = persistBoundsResizeObserved;
    persistBoundsResizeObserved = false;
    if (boundsApproximatelyEqual(next, expectedProgrammaticWindowBounds)) {
      expectedProgrammaticWindowBounds = null;
      return;
    }
    expectedProgrammaticWindowBounds = null;
    const prev = settings.windowBounds || {};
    if (settings?.trayMode) {
      ...
      settings.windowBounds = next;
      if (resized || settings.windowSizeMode === WINDOW_SIZE_MODES.MANUAL) {
        settings.windowSizeMode = WINDOW_SIZE_MODES.MANUAL;
        settings.windowMigrationDone = true;
        settings.manualWindowBounds = next;
      }
    }
    saveSettings();
  }, 400);
...
function readSettings() {
  ...
  try {
    ...
    const windowSizeMigration = migrateWindowSizeSettings(saved);
    merged.windowSizeMode = windowSizeMigration.windowSizeMode;
    merged.windowMigrationDone = windowSizeMigration.windowMigrationDone;
    if (windowSizeMigration.windowBounds) merged.windowBounds = windowSizeMigration.windowBounds;
    if (windowSizeMigration.manualWindowBounds) merged.manualWindowBounds = windowSizeMigration.manualWindowBounds;
    windowSizeMigrationPending = windowSizeMigration.changed;
    ...
    merged.notionTodoEnabled = parseBoolean(merged.notionTodoEnabled, false);
    merged.notionToken = normalizeNotionToken(merged.notionToken);
    merged.notionDataSourceId = normalizeNotionDataSourceId(merged.notionDataSourceId);
    merged.notionTodoRefreshMs = normalizeNotionTodoRefreshMs(merged.notionTodoRefreshMs);
    merged.expenseEnabled = parseBoolean(merged.expenseEnabled, false);
    merged.expenseAutoCategorize = parseBoolean(merged.expenseAutoCategorize, true);
    merged.expenseDefaultCurrency = normalizeCurrency(merged.expenseDefaultCurrency, 'CNY');
    merged.expenseInboxRoot = String(merged.expenseInboxRoot || '').trim();
    merged.calendarLocalAiConfiguration = normalizeCalendarLocalAiConfiguration(merged.calendarLocalAiConfiguration);
    if (calendarLocalAiReadiness(merged.calendarLocalAiConfiguration).status === 'unconfigured') {
      merged.calendarLocalAiConfiguration = verifiedLocalAiConfiguration();
    }
    ...
    merged.windowSizeMode = normalizeWindowSizeMode(merged.windowSizeMode);
    ...
    const defaults = defaultSettings();
    Object.assign(defaults, migrateWindowSizeSettings({}));
    delete defaults.changed;
    delete defaults.reason;
    windowSizeMigrationPending = true;
...
function persistNexaModuleControlPreferences(value) {
  settings = { ...settings, nexaModuleControl: value };
  saveSettings({ throwOnError: true });
}

function resolveCalendarLocalAiCredential(reference) {
  const match = /^env:([A-Z_][A-Z0-9_]{0,127})$/u.exec(String(reference || ''));
  return match ? String(process.env[match[1]] || '') : '';
}

function ensureCalendarLocalAiProviderRuntime() {
  if (!calendarLocalAiProviderRuntime) {
    calendarLocalAiProviderRuntime = createCalendarLocalAiProviderRuntime({
      getConfiguration: () => settings?.calendarLocalAiConfiguration,
      updateConfiguration: async (configuration) => {
        settings = { ...settings, calendarLocalAiConfiguration: normalizeCalendarLocalAiConfiguration(configuration) };
        saveSettings({ throwOnError: true });
        return settings.calendarLocalAiConfiguration;
      },
      fetchImpl: (url, options) => net.fetch(url, options),
      resolveCredential: resolveCalendarLocalAiCredential,
      clock: () => new Date().toISOString()
    });
  }
  return calendarLocalAiProviderRuntime;
}

function ensureLocalAiRuntimeCoordinator() {
  if (!localAiRuntimeCoordinator) {
    localAiRuntimeCoordinator = createSharedLocalAiRuntimeCoordinator({
      provider: ensureCalendarLocalAiProviderRuntime(),
      clock: () => new Date().toISOString()
    });
  }
  return localAiRuntimeCoordinator;
}

function ensureNexaCommandAuditLog() {
  if (!nexaCommandAuditLog) {
    nexaCommandAuditLog = createNexaCommandAuditLog({
      filePath: path.join(app.getPath('userData'), 'nexa', 'local-ai', 'command-audit.jsonl'),
      clock: () => new Date().toISOString()
    });
  }
  return nexaCommandAuditLog;
}
...
async function consumeMobileExpenseBusinessEvent(event) {
  if (!nexaAppComposition?.control) {
    const error = new Error('Consumption composition is not ready');
    error.code = 'CONSUMPTION_NOT_READY';
    throw error;
  }
  const draft = projectMobileEventToExpenseDraft(event);
  await nexaAppComposition.control.startModule('consumption');
  const automatic = draft.confidence >= 0.9;
  const outcome = await nexaAppComposition.control.executeModule('consumption', {
    type: automatic ? 'auto-import-mobile-draft' : 'create-mobile-draft',
    payload: { draft }
  });
  if (!automatic) return { status: 'DRAFTED', reasonCode: 'LOW_CONFIDENCE_PENDING_CONFIRMATION' };
  if (outcome?.skipped === true) {
    return {
      status: 'IGNORED',
      reasonCode: `MOBILE_DRAFT_${String(outcome?.draft?.status || 'SKIPPED')}`
    };
  }
  const confirmedRecordId = typeof outcome?.record?.id === 'string' ? outcome.record.id.trim() : '';
  const confirmedDuplicate = outcome?.duplicate === true &&
    outcome?.draft?.status === 'CONFIRMED' && Boolean(confirmedRecordId);
  if (outcome?.imported === true || confirmedDuplicate) {
    return { status: 'IMPORTED', reasonCode: null };
  }
  const error = new Error('Consumption did not confirm the automatic mobile expense import');
  error.code = 'CONSUMPTION_IMPORT_NOT_CONFIRMED';
  throw error;
}

async function replayPendingMobileExpenseBusinessEvents(store = embeddedHub?.mobileSyncStore) {
  if (!store || typeof store.listPendingBusinessEvents !== 'function') return;
  if (nexaAppComposition?.control) {
    try {
      await nexaAppComposition.control.startModule('consumption');
      await nexaAppComposition.control.executeModule('consumption', {
        type: 'auto-import-pending-mobile-drafts',
        payload: {}
      });
    } catch (_) {
      // Existing pending drafts remain visible as retryable items in Consumption.
    }
  }
  for (const event of store.listPendingBusinessEvents()) {
    try {
      const delivery = await consumeMobileExpenseBusinessEvent(event);
      store.markBusinessEventDelivery(event, delivery.status, delivery.reasonCode || null);
    } catch (error) {
...
function mobileSyncDataFile() {
  return path.join(app.getPath('userData'), 'mobile-sync.json');
}

function temporalIndexDataFile() {
  return path.join(app.getPath('userData'), 'nexa-data-fabric', 'temporal-index-v1.json');
}

function mobileTlsIdentityDirectory() {
  return path.join(app.getPath('userData'), 'mobile-transport', 'tls-identity-v0.1');
}

function mobileHttpsPort(httpPort) {
  return httpPort < 65535 ? httpPort + 1 : httpPort - 1;
}

function desktopMobileAddresses() {
  return [...lanIpv4Addresses(), ...lanIpv6Addresses()];
}

function mobileRelayEnvironmentConfiguration() {
  const host = String(process.env.NEXA_MOBILE_RELAY_HOST || '').trim();
  if (!host) return null;
  if (host.length > 253 || /[\u0000-\u0020\u007f/\\?#@]/.test(host)) {
    throw new Error('NEXA_MOBILE_RELAY_HOST is invalid');
  }
  const port = Number(process.env.NEXA_MOBILE_RELAY_PORT || 17443);
  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error('NEXA_MOBILE_RELAY_PORT is invalid');
  }
  const servername = String(process.env.NEXA_MOBILE_RELAY_SERVERNAME || host).trim();
  if (!servername || servername.length > 253 || /[\u0000-\u0020\u007f/\\?#@]/.test(servername)) {
    throw new Error('NEXA_MOBILE_RELAY_SERVERNAME is invalid');
  }
  const caFile = String(process.env.NEXA_MOBILE_RELAY_CA_FILE || '').trim();
  let ca;
  if (caFile) {
    ca = readRegularFileNoFollow(caFile, {
      encoding: 'utf8',
      description: 'Mobile Relay CA file'
    });
    if (typeof ca !== 'string' || ca.length < 1 || Buffer.byteLength(ca, 'utf8') > 64 * 1024) {
      throw new Error('NEXA Mobile Relay CA file is invalid');
    }
  }
  return Object.freeze({ host, port, servername, ...(ca ? { ca } : {}) });
}

function sendHubPush(payload) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    try { mainWindow.webContents.send('hub:push', stripHubCredentialFields(payload)); } catch (_) {}
  }
}
...
function getHubInfo() {
  ...
  return {
    ...
    port,
    credentialConfigured: Boolean(settings?.hubHostSecret),
    listening: Boolean(embeddedHub),
    listeningPort: embeddedHub ? embeddedHub.port : null,
    mobileHttps: embeddedHub ? {
      listening: true,
      port: embeddedHub.mobileHttpsPort,
      fingerprint_sha256: embeddedHub.mobilePairingRuntime.identity.fingerprint,
      fingerprint_display: embeddedHub.mobilePairingRuntime.identity.fingerprintDisplay,
      expires_at: embeddedHub.mobilePairingRuntime.identity.expiresAt,
      expires_within_30_days: embeddedHub.mobilePairingRuntime.identity.expiresWithin30Days,
      ipv4_listening: Boolean(embeddedHub.hub?.httpsServer?.listening),
      ipv6_listening: Boolean(embeddedHub.hub?.httpsIpv6Server?.listening)
    } : null,
    mobileReverseLan: embeddedHub ? {
      listening_on_mobile_port: REVERSE_LAN_PORT,
      desktop_source_binding: 'PHYSICAL_LAN',
      strategy: TRANSPORT_MODE.AUTO,
      directions: [TRANSPORT_MODE.DIRECT_MOBILE_TO_PC, TRANSPORT_MODE.REVERSE_PC_TO_MOBILE],
      automatic: true
    } : null,
    mobileSecureRelay: embeddedHub ? {
      configured: Boolean(embeddedHub.mobileRelayConfigured),
      priority: 4,
      tls_required: true,
      payload_opaque: true,
      states: embeddedHub.mobileRelayStateProvider?.() || []
    } : null,
    mobileDdns: embeddedHub?.desktopDdnsRuntime?.getState() || {
      state: 'DORMANT',
      hostname: 'pc.6ch3oh.cn',
      ttl: 600
    },
    error: embeddedHubError,
    lanAddresses: desktopMobileAddresses()
  };
}
...
async function startEmbeddedHub() {
  ...
  const port = normalizeHubPort(settings.hubHostPort);
  const httpsPort = mobileHttpsPort(port);
  try {
    const addresses = desktopMobileAddresses();
    const credentialAuthority = createMobileDeviceCredentialAuthority({ credentialStore: ensureCredentialStore() });
    const mobileRelayConfiguration = mobileRelayEnvironmentConfiguration();
    const mobileRelayTransportsByDevice = new Map();
    let mobileRelayFallbackTimer = null;
    let mobileRelayAttemptRunning = false;
    let mobileRelayStopped = false;
    let mobileReverseLanTransport = null;
    const desktopDdnsRuntime = createDesktopDdnsRuntime({
      enabled: false,
      interfacesProvider: lanIpv4Interfaces,
      logger: { log: (message) => console.log(message) }
    });

    function relayRetryDelay() {
      const failed = [...mobileRelayTransportsByDevice.values()].filter((transport) =>
        [RELAY_TRANSPORT_STATES.FAILED, RELAY_TRANSPORT_STATES.DISCONNECTED]
          .includes(transport.getState().state)).length;
      return Math.min(30000, failed ? 1000 * (2 ** Math.min(failed - 1, 5)) : 5000);
    }

    function scheduleMobileRelayFallback(delay = 5000) {
      if (!mobileRelayConfiguration || mobileRelayStopped) return;
      clearTimeout(mobileRelayFallbackTimer);
      mobileRelayFallbackTimer = setTimeout(() => {
        void attemptMobileRelayFallback();
      }, Math.max(0, delay));
      mobileRelayFallbackTimer.unref?.();
    }

    function ensureMobileRelayTransports() {
      if (!mobileRelayConfiguration) return;
      const trustedDevices = credentialAuthority.listDeviceCredentials();
      const controlPending = mobileDeviceControlPlane.pendingCount() > 0;
      const trustedIds = new Set(trustedDevices
        .filter((device) => controlPending || device.connection_state !== 'CONNECTED')
        .map((device) => device.device_id));
      for (const [deviceId, transport] of mobileRelayTransportsByDevice) {
        if (trustedIds.has(deviceId)) continue;
        transport.stop();
        mobileRelayTransportsByDevice.delete(deviceId);
      }
      for (const device of trustedDevices.filter((entry) => trustedIds.has(entry.device_id))) {
        if (mobileRelayTransportsByDevice.has(device.device_id)) continue;
        const transport = createMobileRelayTransport({
          relayHost: mobileRelayConfiguration.host,
          relayPort: mobileRelayConfiguration.port,
          relayServername: mobileRelayConfiguration.servername,
          ...(mobileRelayConfiguration.ca ? { relayCa: mobileRelayConfiguration.ca } : {}),
          localHttpsPort: httpsPort,
          rendezvousIdProvider: () => {
            const token = credentialAuthority.listRelayRendezvousTokens()
              .find((entry) => entry.device_id === device.device_id);
            if (!token) {
              const error = new Error('Relay rendezvous is unavailable');
              error.code = 'RELAY_RENDEZVOUS_UNAVAILABLE';
              throw error;
            }
            return token.rendezvous_id;
          },
          onStateChange: (state) => {
            if ([RELAY_TRANSPORT_STATES.FAILED, RELAY_TRANSPORT_STATES.DISCONNECTED]
              .includes(state)) scheduleMobileRelayFallback(relayRetryDelay());
          },
          logger: { log: (message) => console.log(message) }
        });
        mobileRelayTransportsByDevice.set(device.device_id, transport);
      }
    }

    async function attemptMobileRelayFallback() {
      if (!mobileRelayConfiguration || mobileRelayStopped || mobileRelayAttemptRunning) return;
      mobileRelayAttemptRunning = true;
      try {
        ensureMobileRelayTransports();
        await Promise.allSettled([...mobileRelayTransportsByDevice.values()].map((transport) => {
          const state = transport.getState().state;
          if (state === RELAY_TRANSPORT_STATES.STOPPED) return transport.start();
          if ([RELAY_TRANSPORT_STATES.FAILED, RELAY_TRANSPORT_STATES.DISCONNECTED]
            .includes(state)) return transport.restart();
          return Promise.resolve(transport.getState());
        }));
      } finally {
        mobileRelayAttemptRunning = false;
        scheduleMobileRelayFallback(relayRetryDelay());
      }
    }

    function mobileRelayAuthenticatedPeerCandidate(socket) {
      for (const transport of mobileRelayTransportsByDevice.values()) {
        const candidate = transport.authenticatedPeerCandidate(socket);
        if (candidate) return candidate;
      }
      return null;
    }

    const mobileDeviceControlPlane = createMobileDeviceControlPlane({
      onPendingChange: ({ pending }) => {
        if (pending) {
          void mobileReverseLanTransport?.scanOnce();
          scheduleMobileRelayFallback(0);
        }
      },
      logger: { log: (message) => console.log(message) }
    });
    const mobileDeviceControlClient = createDeviceControlClient({
      controlPlane: mobileDeviceControlPlane,
      trustedDeviceProvider: credentialAuthority.listDeviceCredentials
    });
    credentialAuthority.pruneStalePeerCandidates();
    const mobilePairingRuntime = createMobilePairingRuntime({
      identityDirectory: mobileTlsIdentityDirectory(),
      credentialAuthority,
      endpoint: {
        transport: 'HTTPS',
        host: addresses[0]?.address || '127.0.0.1',
        port: httpsPort
      },
      endpointProvider: () => ({
        transport: 'HTTPS',
        host: lanIpv4Addresses()[0]?.address || lanIpv6Addresses()[0]?.address || '127.0.0.1',
        port: httpsPort
      }),
      lanAddresses: addresses
    });
    let neighborCandidates = [];
    let neighborCandidatesReadAt = 0;
    mobileReverseLanTransport = createMobileReverseLanTransport({
      httpsPort,
      transportMode: TRANSPORT_MODE.AUTO,
      interfacesProvider: lanIpv4Interfaces,
      candidateProvider: async ({ interfaces }) => {
        const current = Date.now();
        if (current - neighborCandidatesReadAt >= 5000) {
          neighborCandidatesReadAt = current;
          neighborCandidates = await windowsArpPeerCandidates();
        }
        const discovered = await discoverReverseLanPeers({ interfaces });
        return [
          ...credentialAuthority.listTrustedPeerCandidates().map((candidate) => ({
            host: candidate.host,
            source: 'TRUSTED_HISTORY'
          })),
          ...neighborCandidates,
          ...discovered
        ];
      },
      pairingActive: mobilePairingRuntime.authority.hasActiveSessions,
      pairedDeviceCount: () => credentialAuthority.listDeviceCredentials().length,
      connectionNeeded: () => mobileDeviceControlPlane.pendingCount() > 0 ||
        credentialAuthority.listDeviceCredentials().some((device) => device.connection_state !== 'CONNECTED'),
      onStateChange: (state) => credentialAuthority.markAllDeviceConnections(state),
      logger: { log: (message) => console.log(message) }
    });
    const mobileSyncStore = createMobileSyncStore({ filePath: mobileSyncDataFile() });
    const mobileTemporalIndex = createNexaTemporalIndex({ filePath: temporalIndexDataFile() });
    const mobileNotificationLedger = createMobileNotificationLedger({
      store: mobileSyncStore,
      provider: ensureCalendarLocalAiProviderRuntime(),
      temporalIndex: mobileTemporalIndex
    });
    let mobileBackgroundCatchupCancelled = false;
    let mobileBackgroundCatchupImmediate = null;
    let mobileBackgroundCatchupPromise = Promise.resolve();
    const runMobileBackgroundCatchup = async () => {
      await mobileNotificationLedger.resumePending();
      const projections = [];
      for (const event of mobileSyncStore.iterateEvents({ eventType: 'RAW_NOTIFICATION' })) {
        if (mobileBackgroundCatchupCancelled) break;
        if (event.notification_classification) {
          projections.push(temporalProjection(event, event.notification_classification));
        }
        if (projections.length >= 100) {
          mobileTemporalIndex.upsert(projections.splice(0), { persist: false });
          await new Promise((resolve) => setImmediate(resolve));
        }
      }
      if (projections.length > 0) mobileTemporalIndex.upsert(projections, { persist: false });
      mobileTemporalIndex.flush();
    };
    const scheduleMobileBackgroundCatchup = () => {
      mobileBackgroundCatchupImmediate = setImmediate(() => {
        mobileBackgroundCatchupImmediate = null;
        mobileBackgroundCatchupPromise = runMobileBackgroundCatchup().catch((error) => {
          console.log(`[mobile-notification-ledger] background catch-up failed: ${error?.code || 'CLASSIFICATION_FAILED'}`);
        });
      });
    };
    const stopMobileBackgroundCatchup = async () => {
      mobileBackgroundCatchupCancelled = true;
      if (mobileBackgroundCatchupImmediate !== null) clearImmediate(mobileBackgroundCatchupImmediate);
      mobileBackgroundCatchupImmediate = null;
      mobileNotificationLedger.pause();
      await mobileNotificationLedger.shutdown();
      await mobileBackgroundCatchupPromise;
      mobileTemporalIndex.flush();
    };
    await replayPendingMobileExpenseBusinessEvents(mobileSyncStore);
    const mobileAwarenessReadModel = createMobileAwarenessReadModel({
      credentialAuthority,
      mobileSyncStore,
      controlClient: mobileDeviceControlClient
    });
    const hub = createHub({
      port,
      host: '0.0.0.0',
      ...
      mobileSyncStore,
      mobileDeviceCredentialAuthority: credentialAuthority,
      mobilePairingAuthority: mobilePairingRuntime.authority,
      mobileTrustedEndpointProvider: (req) => {
        const socketAddress = String(req?.socket?.localAddress || '').replace(/^::ffff:/, '');
        const host = socketAddress && socketAddress !== '127.0.0.1' && socketAddress !== '::1'
          ? socketAddress
          : lanIpv4Addresses()[0]?.address || lanIpv6Addresses()[0]?.address;
        return host ? { transport: 'HTTPS', host, port: httpsPort } : null;
      },
      mobileAuthenticatedPeerResolver: (req) =>
        mobileReverseLanTransport.authenticatedPeerCandidate(req.socket) ||
        mobileRelayAuthenticatedPeerCandidate(req.socket),
      mobileBusinessEventConsumer: consumeMobileExpenseBusinessEvent,
      mobileNotificationEventConsumer: mobileNotificationLedger.enqueue,
      mobileDeviceControlPlane,
      mobileDeviceControlClient,
      mobileRuntimeBundleApplication: createMobileRuntimeBundleApplication(),
      mobileProductGateway: createMobileProductGateway({
        getComposition: () => nexaAppComposition,
        getMobileSyncStore: () => mobileSyncStore,
        onNavigation: openNexaRouteFromMobileCommand
      }),
      desktopSelfStatusProvider: async () => {
        let deviceCenterOverview = null;
        if (nexaAppComposition?.control) {
          try {
            await nexaAppComposition.control.startModule('device-center');
            deviceCenterOverview = await nexaAppComposition.control.executeModule(
              'device-center',
              { operation: 'overview' }
            );
          } catch (_) {
        ...
        return createDesktopSelfStatus({
          identity: desktopIdentity({
            coreDeviceId: settings?.deviceId,
            certificateFingerprint: mobilePairingRuntime.identity.fingerprint,
            displayName: os.hostname()
          }),
          versionName: appVersion(),
          ddnsState: desktopDdnsRuntime.getState(),
          trustedDevices: credentialAuthority.listDeviceCredentials(),
          deviceCenterOverview,
          serviceReady: true
        });
      },
      mobileHttps: {
        ...
        host: '0.0.0.0',
        ipv6Host: '::',
        key: mobilePairingRuntime.identity.privateKeyPem,
        cert: mobilePairingRuntime.identity.certificatePem
      },
      requireHttpsForMobile: true,
      logger: { error: (err) => console.log(`[hub] ${err?.message || err}`) }
    });
    await hub.start();
    scheduleMobileBackgroundCatchup();
    desktopDdnsRuntime.start();
    mobileReverseLanTransport.start();
    scheduleMobileRelayFallback(5000);
    embeddedHub = {
      hub,
      port,
      mobileHttpsPort: httpsPort,
      mobilePairingRuntime,
      mobileReverseLanTransport,
      desktopDdnsRuntime,
      mobileRelayConfigured: Boolean(mobileRelayConfiguration),
      mobileRelayStateProvider: () => [...mobileRelayTransportsByDevice.values()]
        .map((transport) => transport.getState().state),
      stopMobileRelay: () => {
        mobileRelayStopped = true;
        clearTimeout(mobileRelayFallbackTimer);
        mobileRelayFallbackTimer = null;
        for (const transport of mobileRelayTransportsByDevice.values()) transport.stop();
        mobileRelayTransportsByDevice.clear();
      },
      mobileDeviceControlPlane,
      mobileDeviceControlClient,
      mobileAwarenessReadModel,
      mobileSyncStore,
      stopMobileBackgroundCatchup
    };
    console.log(`[hub] listening on 0.0.0.0:${port}`);
    console.log(`[hub] mobile HTTPS listening on 0.0.0.0:${httpsPort}`);
    if (hub.httpsIpv6Server?.listening) console.log(`[hub] mobile HTTPS IPv6 listening on [::]:${httpsPort}`);
...
async function stopEmbeddedHub() {
  ...
  embeddedHub = null;
  try { handle.stopMobileRelay?.(); } catch (_) {}
  try { handle.desktopDdnsRuntime?.stop(); } catch (_) {}
  try { handle.mobileReverseLanTransport?.stop(); } catch (_) {}
  try { handle.mobileDeviceControlPlane?.shutdown(); } catch (_) {}
  try { handle.mobilePairingRuntime.shutdown(); } catch (_) {}
  try { await handle.hub.stop(); } catch (_) {}
  try { await handle.stopMobileBackgroundCatchup?.(); } catch (_) {}
  sendHubPush({ type: 'stopped', info: getHubInfo() });
}

function requireMobilePairingRuntime() {
  if (settings?.hubMode !== 'host' || !embeddedHub?.mobilePairingRuntime) {
  ...
  return embeddedHub.mobilePairingRuntime;
}

function requireMobileAwarenessReadModel() {
  requireMobilePairingRuntime();
  if (!embeddedHub?.mobileAwarenessReadModel) {
    const error = new Error('Mobile awareness is unavailable');
    error.code = 'MOBILE_AWARENESS_UNAVAILABLE';
    throw error;
  }
  return embeddedHub.mobileAwarenessReadModel;
}

async function rotateMobileCertificate(confirmed) {
  if (confirmed !== true) throw new Error('explicit certificate rotation confirmation is required');
  const runtime = requireMobilePairingRuntime();
  runtime.authority.revokeAllPairedDevices();
  await stopEmbeddedHub();
  rotateMobileTlsIdentity(mobileTlsIdentityDirectory(), {
    confirmed: true,
    lanAddresses: desktopMobileAddresses()
  });
  const handle = await startEmbeddedHub();
  if (!handle) throw new Error('mobile HTTPS listener failed to restart after certificate rotation');
  return handle.mobilePairingRuntime.certificateInfo();
}
...
function settingsForRenderer() {
  const rendererSettings = stripCredentialSettings(settings || {});
  delete rendererSettings.nexaModuleControl;
  ...
  const kimiWebAccessTokenSource = settings?.kimiWebAccessToken
    ...
    : kimiWebToken(process.env)
      ? 'env'
      : '';
  // Default-deny every credential field added to the canonical store. Hub
  // credentials stay in the privileged process; the renderer receives status
  // only and may submit a replacement without receiving the stored value.
  const redactedCredentials = credentialSettingsForRenderer(settings, {
    omit: ['hubHostSecret', 'secret']
  });
  return stripHubCredentialFields({
    ...rendererSettings,
    locale: trayMenuLocale(),
    ...redactedCredentials,
    hubCredentialConfigured: Boolean(settings?.hubHostSecret),
    clientCredentialConfigured: Boolean(settings?.secret),
    ...notionTodoSettingsForRenderer(settings),
    expenseEnabled: Boolean(settings?.expenseEnabled),
    expenseAutoCategorize: Boolean(settings?.expenseAutoCategorize),
    expenseDefaultCurrency: normalizeCurrency(settings?.expenseDefaultCurrency, 'CNY'),
    expenseInboxRoot: String(settings?.expenseInboxRoot || '').trim(),
    expenseCategories: DEFAULT_CATEGORY_RULES.map((rule) => ({ category: rule.category, label: rule.label })),
...
function notionTodoCachePath() {
  return path.join(app.getPath('userData'), 'notion-todos.json');
}

function sendNotionTodoPush(payload) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    try { mainWindow.webContents.send('notionTodo:push', payload); } catch (_) {}
  }
}

function ensureNotionTodoRuntime() {
  if (notionTodoRuntime) return notionTodoRuntime;
  notionTodoRuntime = createNotionTodoRuntime({
    getSettings: () => settings || {},
    cachePath: notionTodoCachePath(),
    fs,
    logger: (line) => console.log(line),
    onUpdate: sendNotionTodoPush
  });
  return notionTodoRuntime;
}

function reconfigureNotionTodoRuntime() {
  if (!app.isReady()) return;
  ensureNotionTodoRuntime().reconfigure();
}

function expenseDataRoot() {
  // Prefer an explicit operator-set inbox root (i.e. `.env`), otherwise the user-data
  // expense dir. The root is always resolved and, when overridden, can point anywhere the
  // operator chose; the default stays inside userData so the inbox is private by default.
  const configured = String(settings?.expenseInboxRoot || '').trim();
  if (configured) return path.resolve(configured);
  return path.join(app.getPath('userData'), 'expense');
}

function sendExpensePush(payload) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    try { mainWindow.webContents.send('expense:push', payload); } catch (_) {}
  }
}

function ensureExpenseRuntime() {
  if (expenseRuntime) return expenseRuntime;
  expenseRuntime = createExpenseRuntime({
    root: expenseDataRoot(),
    inboxName: DEFAULT_INBOX_NAME,
    recordsName: 'expense-records.json',
    fs,
    logger: (line) => console.log(line),
    getSettings: () => settings || {},
    onUpdate: sendExpensePush
  });
  return expenseRuntime;
}

function reconfigureExpenseRuntime() {
  if (!app.isReady()) return;
  try {
    ensureExpenseRuntime().reconfigure();
  } catch (error) {
    console.log(`[expense] reconfigure failed: ${error.message}`);
  }
}
...
function openNexaRouteFromMobileCommand(routeId) {
  const normalized = String(routeId || '').trim().toLowerCase();
  if (!NEXA_MOBILE_COMMAND_ROUTE_IDS.has(normalized)) {
    console.warn('[mobile-product] operation=global-command.submit outcome=navigation result=REJECTED_INVALID_ROUTE');
    throw Object.assign(new TypeError('Mobile navigation target is not a registered NEXA route'), {
      code: 'MOBILE_NAVIGATION_TARGET_INVALID'
    });
  }
  focusExistingWindow();
  sendMainWindowEvent('view:open', Object.freeze({
    source: 'mobile-global-command',
    route_id: normalized
  }));
  console.log(`[mobile-product] operation=global-command.submit outcome=navigation route=${normalized} result=DISPATCHED`);
}
...
function startNexaShellBridgeSafely() {
  if (nexaShellStartPromise) return nexaShellStartPromise;

  let attempt;
  attempt = Promise.resolve()
    .then(() => createNexaAppComposition({
      consumptionRepositoryPath: path.join(
        app.getPath('userData'),
        'nexa',
        'consumption',
        'expense-records.json'
      ),
      todayTomorrowDataRoot: path.join(
        app.getPath('userData'),
        'nexa',
        'today-tomorrow'
      ),
      deviceCenterDataRoot: path.join(
        app.getPath('userData'),
        'nexa',
        'device-center'
      ),
      automationCenterDataRoot: path.join(
        app.getPath('userData'),
        'nexa',
        'automation-center'
      ),
      marketDataRoot: path.join(app.getPath('userData'), 'nexa', 'market'),
      starBenchDataRoot: path.join(app.getPath('userData'), 'nexa', 'starbench'),
      studyCenterDataRoot: process.env.NEXA_STUDY_CENTER_DATA_ROOT || undefined,
      marketPythonExecutable: 'python',
      marketRuntimeMode: 'EMPTY',
      timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC',
      clock: () => new Date().toISOString(),
      calendarLocalAiProvider: ensureCalendarLocalAiProviderRuntime(),
      localAiRuntimeCoordinator: ensureLocalAiRuntimeCoordinator(),
      commandAuditLog: ensureNexaCommandAuditLog(),
      localSpeechToTextProvider: createLocalSpeechToTextProvider(),
      getDeviceSnapshot: () => deviceRuntimeHandle?.getSnapshot?.(),
      getHubStats: () => latestStats,
      toggleWindowMaximize: (event) => {
        const targetWindow = BrowserWindow.fromWebContents(event?.sender);
        if (!targetWindow || targetWindow.isDestroyed() || settings?.trayMode) return false;
        if (targetWindow.isMaximized()) targetWindow.unmaximize();
        else targetWindow.maximize();
        return true;
      },
      openCreatorOpsEndpoint: (url) => shell.openExternal(url),
      showLocalResourceDialog: (event, options) => {
        const parentWindow = BrowserWindow.fromWebContents(event.sender);
        return parentWindow
          ? dialog.showOpenDialog(parentWindow, options)
          : dialog.showOpenDialog(options);
      },
      lstatLocalResourcePath: (absolutePath) => fs.promises.lstat(absolutePath),
      mobilePairingApplication: Object.freeze({
        start: async () => stripHubCredentialFields(await requireMobilePairingRuntime().startPairing()),
        status: async (pairingId) => stripHubCredentialFields(
          await requireMobilePairingRuntime().status(pairingId)
        ),
        cancel: async (pairingId) => stripHubCredentialFields(
          await requireMobilePairingRuntime().cancel(pairingId)
        ),
        desktopConfirm: async (pairingId, confirmed) => stripHubCredentialFields(
          await requireMobilePairingRuntime().desktopConfirm(pairingId, confirmed)
        ),
        listDevices: async () => stripHubCredentialFields(
          await requireMobilePairingRuntime().pairedDevices()
        ),
        revoke: async (deviceReference) => stripHubCredentialFields(
          await requireMobilePairingRuntime().revokeDeviceReference(deviceReference)
        ),
        certificate: async () => stripHubCredentialFields(
          await requireMobilePairingRuntime().certificateInfo()
        ),
        rotateCertificate: async (confirmed) => stripHubCredentialFields(
          await rotateMobileCertificate(confirmed)
        ),
        awareness: (options) => requireMobileAwarenessReadModel().read(options),
        awarenessAction: (request) => requireMobileAwarenessReadModel().action(request),
        shutdown: () => embeddedHub?.mobilePairingRuntime.shutdown()
      }),
      moduleControlPreferences: settings?.nexaModuleControl,
      persistModuleControlPreferences: persistNexaModuleControlPreferences
    }))
    .then(async (composition) => {
      nexaAppComposition = composition;
      nexaDeviceCenterActivityController.sync();
      void ensureLocalAiRuntimeCoordinator().start().then((result) => {
        console.log(`[local-ai] coordinator result=${result.status} ownership=${result.ownership || 'NONE'} reason=${result.last_reason || 'NONE'}`);
      });
      nexaShellBridge = createEmptyNexaElectronShellBridge({ host: composition.control });
      return nexaShellBridge.start();
    })
    .then(() => applyNexaIpcRegistrationPlan(
      nexaAppComposition.registrationPlan,
      ipcMain
    ))
    .then((registrationHandle) => {
      nexaIpcRegistrationHandle = registrationHandle;
      return nexaAppComposition.control.startAutoModules();
    })
    .then((result) => {
      if (result?.failedModuleIds?.length) {
        console.log(`[nexa-shell] autoStart failed for: ${result.failedModuleIds.join(', ')}`);
      }
      return nexaAppComposition.control.startModule('automation-center').catch((error) => {
        console.log(`[nexa-shell] automation-center host unavailable: ${error?.code || 'START_FAILED'}`);
      });
    })
    .then(() => undefined)
    .catch((error) => {
      console.log(`[nexa-shell] startup failed: ${error?.message || error}`);
    })
    .finally(() => {
      if (nexaShellStartPromise === attempt) nexaShellStartPromise = null;
    });
  nexaShellStartPromise = attempt;
  return attempt;
}

async function stopNexaShellBridgeSafely() {
  if (nexaShellStartPromise) await nexaShellStartPromise;
  if (nexaIpcRegistrationHandle) {
    await nexaIpcRegistrationHandle.dispose();
    nexaIpcRegistrationHandle = null;
  }
  if (nexaShellBridge) await nexaShellBridge.stop();
  await localAiRuntimeCoordinator?.stop?.();
}

function runLegacyQuitCleanupOnce() {
  if (legacyQuitCleanupComplete) return;
  legacyQuitCleanupComplete = true;
  notionTodoRuntime?.stop();
...
function handleBeforeQuit(event) {
  quitRequested = true;
  if (nexaShellShutdownApproved) return;
  event.preventDefault();
  if (nexaShellQuitPromise) return;

  let attempt;
  attempt = Promise.resolve()
    .then(() => stopNexaShellBridgeSafely())
    .then(() => {
      runLegacyQuitCleanupOnce();
      nexaShellShutdownApproved = true;
      if (nexaShellQuitPromise === attempt) nexaShellQuitPromise = null;
      app.quit();
    })
    .catch((error) => {
      quitRequested = false;
      console.log(`[nexa-shell] shutdown failed: ${error?.message || error}`);
    })
    .finally(() => {
      if (nexaShellQuitPromise === attempt) nexaShellQuitPromise = null;
    });
  nexaShellQuitPromise = attempt;
}
...
function createWindow(boundsOverride, options = {}) {
  ...
  nexaDeviceCenterActivityController.bind(win);
  ...
  win.on('maximize', () => {
    if (applyingWindowSizeMode) return;
  ...
  win.on('resize', () => persistBoundsSoon({ resized: true }));
  win.on('resized', () => persistBoundsSoon({ resized: true }));
...
app.whenReady().then(async () => {
  ...
  ensureSettingsLoaded();
  await startNexaShellBridgeSafely();
  ...
  applyMacActivationPolicy();
  createWindow();
  screen.on('display-added', clampMainWindowToVisibleWorkArea);
  screen.on('display-removed', clampMainWindowToVisibleWorkArea);
  screen.on('display-metrics-changed', clampMainWindowToVisibleWorkArea);
  ...
  startMode();
  ensureNotionTodoRuntime().start();
  try { ensureExpenseRuntime().start(); } catch (error) { console.log(`[expense] start failed: ${error.message}`); }
  ...
  ipcMain.handle('settings:update', (_event, patch) => {
    ...
    const previousNotionTodoSignature = JSON.stringify({
      enabled: Boolean(settings.notionTodoEnabled),
      token: settings.notionToken || '',
      dataSourceId: settings.notionDataSourceId || '',
      refreshMs: settings.notionTodoRefreshMs || 0
    });
    const previousExpenseEnabled = Boolean(settings.expenseEnabled);
    const previousExpenseAutoCategorize = Boolean(settings.expenseAutoCategorize);
    const previousExpenseRoot = String(settings.expenseInboxRoot || '');
    ...
    delete normalizedPatch.notionTokenConfigured;
    delete normalizedPatch.nexaModuleControl;
    if (patch.calendarLocalAiConfiguration !== undefined) {
      normalizedPatch.calendarLocalAiConfiguration = normalizeCalendarLocalAiConfiguration(
        patch.calendarLocalAiConfiguration
      );
    }
    ...
    if (patch.notionToken !== undefined) normalizedPatch.notionToken = normalizeNotionToken(patch.notionToken);
    if (patch.notionDataSourceId !== undefined) normalizedPatch.notionDataSourceId = normalizeNotionDataSourceId(patch.notionDataSourceId);
    if (patch.notionTodoRefreshMs !== undefined) normalizedPatch.notionTodoRefreshMs = normalizeNotionTodoRefreshMs(patch.notionTodoRefreshMs);
    if (patch.expenseEnabled !== undefined) normalizedPatch.expenseEnabled = parseBoolean(patch.expenseEnabled, false);
    if (patch.expenseAutoCategorize !== undefined) normalizedPatch.expenseAutoCategorize = parseBoolean(patch.expenseAutoCategorize, true);
    if (patch.expenseDefaultCurrency !== undefined) normalizedPatch.expenseDefaultCurrency = normalizeCurrency(patch.expenseDefaultCurrency, 'CNY');
    ...
    if (patch.windowSizeMode !== undefined) normalizedPatch.windowSizeMode = normalizeWindowSizeMode(patch.windowSizeMode, settings.windowSizeMode);
    settings = normalizeWindowBehaviorSettings({
      ...
      notionTodoEnabled: parseBoolean(patch.notionTodoEnabled ?? settings.notionTodoEnabled, false),
      notionToken: patch.notionToken !== undefined ? normalizeNotionToken(patch.notionToken) : (settings.notionToken || ''),
      notionDataSourceId: patch.notionDataSourceId !== undefined ? normalizeNotionDataSourceId(patch.notionDataSourceId) : normalizeNotionDataSourceId(settings.notionDataSourceId),
      notionTodoRefreshMs: patch.notionTodoRefreshMs !== undefined ? normalizeNotionTodoRefreshMs(patch.notionTodoRefreshMs) : normalizeNotionTodoRefreshMs(settings.notionTodoRefreshMs),
      expenseEnabled: parseBoolean(patch.expenseEnabled ?? settings.expenseEnabled, false),
      expenseAutoCategorize: parseBoolean(patch.expenseAutoCategorize ?? settings.expenseAutoCategorize, true),
      expenseDefaultCurrency: patch.expenseDefaultCurrency !== undefined ? normalizeCurrency(patch.expenseDefaultCurrency, 'CNY') : normalizeCurrency(settings.expenseDefaultCurrency, 'CNY'),
      expenseInboxRoot: patch.expenseInboxRoot !== undefined ? String(patch.expenseInboxRoot || '').trim() : String(settings.expenseInboxRoot || '').trim(),
      ...
      calendarLocalAiConfiguration: patch.calendarLocalAiConfiguration !== undefined
        ? normalizeCalendarLocalAiConfiguration(patch.calendarLocalAiConfiguration)
        : normalizeCalendarLocalAiConfiguration(settings.calendarLocalAiConfiguration),
    ...
    const nextWindowSizeBounds = patch.windowSizeMode !== undefined
      ? prepareWindowSizeMode(settings.windowSizeMode)
      : null;
    ...
    if (nextWindowSizeBounds) setMainWindowBoundsPreservingMaximize(nextWindowSizeBounds);
    ...
    const nextNotionTodoSignature = JSON.stringify({
      enabled: Boolean(settings.notionTodoEnabled),
      token: settings.notionToken || '',
      dataSourceId: settings.notionDataSourceId || '',
      refreshMs: settings.notionTodoRefreshMs || 0
    });
    if (nextNotionTodoSignature !== previousNotionTodoSignature) reconfigureNotionTodoRuntime();
    const nextExpenseEnabled = Boolean(settings.expenseEnabled);
    const nextExpenseAutoCategorize = Boolean(settings.expenseAutoCategorize);
    const nextExpenseRoot = String(settings.expenseInboxRoot || '');
    if (
      nextExpenseEnabled !== previousExpenseEnabled
      || nextExpenseAutoCategorize !== previousExpenseAutoCategorize
      || nextExpenseRoot !== previousExpenseRoot
    ) reconfigureExpenseRuntime();
  ...
  ipcMain.on('window:viewState', (_event, patch) => {
    const mobileNavigation = patch?.mobileCommandNavigation;
    const mobileRouteId = typeof mobileNavigation?.route_id === 'string'
      ? mobileNavigation.route_id.trim().toLowerCase()
      : '';
    const mobileResult = mobileNavigation?.result;
    if (NEXA_MOBILE_COMMAND_ROUTE_IDS.has(mobileRouteId) && ['APPLIED', 'NOT_APPLIED'].includes(mobileResult)) {
      console.log(`[mobile-product] operation=global-command.submit renderer=${mobileResult} route=${mobileRouteId}`);
    }
  ...
  ipcMain.handle('hub:getInfo', () => stripHubCredentialFields(getHubInfo()));
  ...
  ipcMain.handle('notionTodo:get', () => ensureNotionTodoRuntime().getSnapshot());
  ipcMain.handle('notionTodo:refresh', () => ensureNotionTodoRuntime().refresh({ force: true }));
  ipcMain.handle('notionTodo:test', () => ensureNotionTodoRuntime().testConnection());
  ipcMain.handle('notionTodo:open', (_event, url) => {
    if (!isAllowedNotionTodoUrl(url)) return { ok: false, error: 'url not in allowlist' };
    return shell.openExternal(url)
      .then(() => ({ ok: true }))
      .catch(() => ({ ok: false, error: 'open failed' }));
  });
  ipcMain.handle('app:openUserData', () => shell.openPath(app.getPath('userData')));
  ipcMain.handle('expense:get', () => {
    try { return ensureExpenseRuntime().getSnapshot(); }
    catch (error) { return { ok: false, error: error.message }; }
  });
  ipcMain.handle('expense:records', () => {
    try { return ensureExpenseRuntime().records(); }
    catch (error) { return { ok: false, error: error.message }; }
  });
  ipcMain.handle('expense:refresh', () => {
    try { ensureExpenseRuntime().reconfigure(); return ensureExpenseRuntime().getSnapshot(); }
    catch (error) { return { ok: false, error: error.message }; }
  });
  ipcMain.handle('expense:openInbox', async () => {
    try {
      const info = ensureExpenseRuntime().getInboxInfo();
      await shell.openPath(info.inboxDir);
      return { ok: true, inboxDir: info.inboxDir };
    } catch (error) { return { ok: false, error: error.message }; }
  });
  ipcMain.handle('expense:openExpenseRoot', async () => {
    try {
      const root = expenseDataRoot();
      await shell.openPath(root);
      return { ok: true, root };
    } catch (error) { return { ok: false, error: error.message }; }
  });
  ipcMain.handle('expense:importCsvPreview', (_event, text) => {
    try { return ensureExpenseRuntime().importPreview(String(text || '')); }
    catch (error) { return { ok: false, error: error.message }; }
  });
  ipcMain.handle('expense:importCsvConfirm', (_event, drafts) => {
    try { return ensureExpenseRuntime().importConfirm(drafts); }
    catch (error) { return { ok: false, error: error.message }; }
  });
  ipcMain.handle('expense:clear', () => {
    try { return ensureExpenseRuntime().clear(); }
    catch (error) { return { ok: false, error: error.message }; }
  });
  ...
  powerMonitor.on('resume', () => {
    void localAiRuntimeCoordinator?.notifyResume?.().then((state) => {
      console.log(`[local-ai] resume-recovery result=${state.status} reason=${state.last_reason || 'NONE'}`);
    });
  });
