...
## `POST /nexa/mobile/sync`

Receives a NEXA Mobile business-sync batch using the frozen `nexa.mobile.sync.v1` contract. This route uses a per-device credential, not the Hub-wide widget secret:
...
The credential is resolved by the shared Core device credential authority and must be bound to the request's stable `device_id`. Missing or invalid credentials are rejected with `401`; a valid credential claiming another `device_id` is rejected with `403`. Raw credentials are never stored or logged: only a verifier digest is retained in the existing private atomic `credentials.json` store. The same authority is designed for reuse by the future status receiver.

Request:

```json
{
  "protocol_version": "nexa.mobile.sync.v1",
  "device_id": "device-install-1",
  "batch_id": "01J1ZPQ8A3M5G8XZ3Q7N4D0T4M",
  "sent_at": 1787529600000,
  "events": [
    {
      "event_type": "RAW_NOTIFICATION",
      "event_id": "01J1ZPQ8A3M5G8XZ3Q7N4D0T4N",
      "payload": {
        "raw_text": "..."
      },
      "event_time": 1787529599000,
      "attempt": 1
    }
  ]
}
...
- `protocol_version` must be `nexa.mobile.sync.v1`.
- `device_id`, `batch_id`, `sent_at`, and all event fields use the frozen snake_case wire names.
- A batch may contain 1 to 50 events.
- `event_type` is `RAW_NOTIFICATION` or `PARSED_TRANSACTION`.
- Each `(event_type, event_id)` must be unique within a batch.
- Each `payload` must be a non-empty JSON object; `event_time` is a non-negative integer and `attempt` is 1 through 20.
- The receiver's per-event payload safety limit is 256 KiB. An identifiable event over that limit is acknowledged as `REJECTED` and is not persisted.
- The durable idempotency identity is `(device_id, event_type, event_id)`. A retransmission returns `DUPLICATE` without a second write; the same type and ID from a different device remains independent.
...
Every identifiable event receives exactly one acknowledgement with `ACCEPTED`, `DUPLICATE`, or `REJECTED`. Rejections carry a stable, non-sensitive `reason`; rejected payloads are not persisted. Malformed envelopes, missing identities, unsupported protocol or event types, non-snake-case aliases, and invalid event-array shapes return non-2xx errors before persistence.

The former WIP route `POST /api/mobile-sync/v1/batches`, its camelCase request, and its split `accepted[]` / `duplicates[]` / `rejected[]` response are retired. Version-1 store records cannot be safely assigned a `device_id` or `event_type`; if encountered, they are preserved unchanged under an isolated legacy section during the next atomic write and never guessed into the new identity space.

## `PUT /nexa/mobile/sync/status`

Receives the latest read-only NEXA Mobile Capture Desktop status projection using contract `nexa.mobile.capture.desktop-handoff.v0.1`. It is a distinct request and ACK inside the same Mobile protocol family, Hub lifecycle, per-device credential authority, and atomic Mobile persistence authority as the business endpoint.

The request must use `Content-Type: application/json; charset=utf-8` and the same per-device Bearer credential model as business sync. The authenticated device identity must exactly match `identity.device_id`; the Hub-wide secret cannot substitute for that credential.

Request:

```json
{
  "contract_version": "nexa.mobile.capture.desktop-handoff.v0.1",
  "captured_at_epoch_ms": 1700000000123,
  "identity": {
    "device_id": "device-install-ready",
    "application_id": "com.xingshu.nexa.mobile",
    "version_name": "0.1.0",
    "version_code": 1
  },
  "capture": {
    ...
    "notification_listener_permission_granted": true
  },
  "sync": {
    "status": "READY",
    "readiness": "READY",
    ...
    "last_sync_at_epoch_ms": 1700000000000,
    "next_retry_at_epoch_ms": null
  },
  "diagnostic": {
    "summary": "DATA_PENDING",
    "reason_code": null,
    "last_activity_at_epoch_ms": 1700000000100
  }
}
```

The receiver rejects unknown fields and alternative casing. It reconstructs the frozen deterministic snake_case JSON projection, hashes its UTF-8 bytes with SHA-256, and stores only that safe snapshot plus its 64-character lowercase fingerprint under `latest_status_by_device` in the existing `mobile-sync.json` atomic store. Credentials, authorization headers, notification or business payloads, event/queue IDs, endpoint configuration, certificates, repository objects, and scheduler commands are never part of this section.

Ordering is independent per `device_id` and uses only `captured_at_epoch_ms`:

- A first or newer snapshot is atomically stored and returns `APPLIED` with HTTP `200`.
- An older snapshot is not stored and returns `STALE_IGNORED` with HTTP `200` and reason `OLDER_THAN_CURRENT_SNAPSHOT`.
- The same timestamp and fingerprint is not applied or persisted again and returns `DUPLICATE` with HTTP `200` and reason `IDENTICAL_SNAPSHOT`.
- The same timestamp with a different fingerprint is not stored and returns `TIMESTAMP_CONFLICT` with HTTP `409` and reason `EQUAL_TIMESTAMP_DIFFERENT_SNAPSHOT`.

ACK fields are exactly:
...
This stored snapshot is not evidence that the phone, PC, or network is online, paired, reachable, or healthy. Future presentation layers must use a separate freshness contract before describing it as current, stale, or unavailable.

The general Hub surface may continue to serve its existing HTTP clients, but every Mobile pairing, business-sync, and status request is accepted only by the separate embedded HTTPS listener. That listener uses the persistent Desktop certificate identity; Mobile must pin its SHA-256 fingerprint and present the per-device credential bound to `device_id`.

## `POST /nexa/mobile/control/exchange`

Carries the bounded `nexa.device.control.v0.1` request/response exchange over the same Mobile HTTPS listener, pinned Desktop certificate, per-device Bearer credential, and `device_id` binding as Sync and Status. It is not an HTTP shell or a general remote-execution API. The frozen capability allowlist is limited to safe device, network, sync, capture, reconnect, transport-reevaluation, and bounded redacted diagnostic operations. Unknown capabilities, extra fields, stale or mismatched request identities, oversized exchanges, and untrusted transport sources fail closed.

Only one request may be pending per trusted device. Commands expire after 60 seconds and are never persisted as a second durable queue. A Mobile response must match both `request_id` and `device_id`. Diagnostic bundles are private-app artifacts capped at 128 KiB with bounded count and lifetime; credentials, authorization headers, certificate keys, notification content, arbitrary filesystem content, logcat, shell commands, intents, and URLs are outside the contract.

The exchange endpoint accepts authenticated requests only from a verified Direct, Reverse, Campus Routed, or Secure Relay transport. Relay presence alone grants no control authority: the inner pinned TLS session, Bearer credential, `device_id`, exact capability schema, and request identity remain mandatory.

## NEXA Mobile trusted multi-network transport

The production `AUTO` priority is frozen inside the same HTTPS, pairing, credential, certificate, Sync, Status, and Control protocol family:

- `DIRECT_MOBILE_TO_PC`: Mobile opens the Desktop HTTPS endpoint directly.
- `REVERSE_PC_TO_MOBILE`: Desktop opens a bounded candidate on the Mobile reverse port and bridges that stream to the same local HTTPS listener.
- `CAMPUS_ROUTED_DIRECT`: Mobile tries only previously authenticated, live routed IPv4/IPv6 endpoint candidates over the system network.
- `SECURE_RELAY`: both peers open outbound TLS to an opaque rendezvous Relay; the unchanged inner pinned Desktop TLS stream passes through it.

An authenticated existing connection wins. On network changes, `AUTO` re-evaluates in the order Direct, Reverse, Campus Routed, then Relay. Relay is a fallback, not the default. `LOCAL_DIRECT` remains local-only; `FOLLOW_SYSTEM` uses routed candidates and may fall back to a configured Relay. This choice is transport metadata only; it never creates another device identity, credential authority, business protocol, or sync queue.

The QR payload is `FIRST_PAIRING_BOOTSTRAP_ONLY`. A successful pairing persists the existing device identity and credential verifier, while the Desktop TLS identity remains in its existing private identity store. Desktop retains only bounded recently authenticated endpoint candidates. These are discovery hints, not trust: an IP address is never a device identity, and a candidate is stored or refreshed only after HTTPS credential authentication has already bound the request to its `device_id`. Revocation removes both the credential binding and its candidate metadata.

Desktop discovery is local, bounded, and non-secret. It combines recent authenticated candidates, at most 24 entries from the Windows neighbor cache, and the following directed-broadcast probe. It never enumerates a `/24` or modifies routes, adapters, VPN settings, or firewall rules.

```text
UDP destination: physical-interface directed broadcast, port 17323
Request:  NEXA_REVERSE_LAN_DISCOVERY_V1 DISCOVER <base64url-nonce>\n
Response: NEXA_REVERSE_LAN_DISCOVERY_V1 AVAILABLE <same-nonce> 17324\n
```

Responses are limited to 128 bytes, accepted only from a private address in the source physical interface's subnet, nonce-bound, deduplicated, and capped at eight peers. A response grants no trust and carries no credential.

For a reverse connection, Desktop opens TCP port `17324` with `localAddress` fixed to the matching physical WLAN or Ethernet address. Mobile must immediately write this exact ASCII preface:

```text
NEXA_REVERSE_LAN_V1\n
```

The bytes after that preface are the unchanged TLS client stream for the existing Desktop HTTPS listener. Desktop preserves any TLS bytes received in the same TCP frame, rejects an invalid or oversized preface, classifies failures as `TCP_CONNECT`, `REVERSE_PREFACE`, or `LOCAL_HTTPS_CONNECT`, and closes a reverse bridge that does not produce a successfully credential-authenticated HTTPS request within five seconds. Certificate pinning, `device_id`, and the per-device credential remain mandatory in both directions.

Campus-routed candidates have a 30-minute default lifetime, a 24-hour hard maximum lifetime, and a maximum of eight entries on Mobile. They may contain safe routed unicast IPv4, IPv6 global/ULA/link-local addresses, or restricted `.local`/`.lan` names. Loopback, unspecified, multicast, reserved, credential-bearing, path/query/fragment, malformed, and unsafe hostname candidates are rejected. Desktop advertises the actual authenticated HTTPS listener address, including bracketed IPv6 when applicable. Direct transport keeps private/local source restrictions; routed global addresses are accepted only after existing TLS and credential authentication succeeds. No subnet or campus-wide scan is performed.

Secure Relay V0.1 uses outer TLS 1.2 or newer and one length-prefixed registration frame capped at 2 KiB. Its exact JSON fields are `contract_version`, `role`, `rendezvous_id`, `issued_at_epoch_ms`, and `nonce`. The rendezvous identifier is a five-minute HMAC-derived value from the existing credential verifier; the raw credential, verifier digest, device identity, certificate, endpoint, and business metadata are never registration fields. Registrations allow 30 seconds of clock skew and nonces are replay-bounded.

After matching one `DESKTOP` and one `MOBILE` role, Relay forwards opaque bytes only. It has bounded clients, per-address registration rate, wait TTL, nonce lifetime, session lifetime, 30-second idle timeout, and a 16 MiB session byte budget. It stores no business events or diagnostics. The inner Desktop certificate pin, Bearer credential, and `device_id` remain the actual peer authority, so an untrusted or compromised Relay can deny service and observe timing/byte counts but cannot decrypt a valid inner session or issue Control commands.

The standalone Relay is started with `npm run relay -- --host <host> --port 17443 --key <tls-key.pem> --cert <tls-cert.pem>`. Desktop is enabled when `NEXA_MOBILE_RELAY_HOST` is configured; `NEXA_MOBILE_RELAY_PORT` defaults to `17443`, `NEXA_MOBILE_RELAY_SERVERNAME` defaults to the host, and optional `NEXA_MOBILE_RELAY_CA_FILE` adds a private CA while an omitted file uses the platform trust store. Mobile receives the same non-secret infrastructure endpoint at build time through Gradle properties `nexaMobileRelayHost`, `nexaMobileRelayPort`, and `nexaMobileRelayServerName`. Empty Mobile properties and an absent Desktop host leave Relay `NOT_CONFIGURED` without weakening local paths.
